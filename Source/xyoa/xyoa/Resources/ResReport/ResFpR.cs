namespace xyoa.Resources.ResReport
{
    ...
    public class ResFpR : Page
    {
        ...
        public void Bindquanxian()
        {
            ...
            this.pulicss.QuanXianVis(this.GridView1, "bbbb3a", this.Session["perstr"].ToString());
        }
        ...
        {
            base.Response.Redirect("ResFpR.aspx");
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.JbName,A.GmTime,A.GmNum,A.JsRealname,A.JsUsername,A.States, C.ZyNum, C.ZyName, B.Name as CangkuName, E.Name as QuYuName,D.Name as ZyLeibieName from [qp_oa_ResFp] as [A] inner join [qp_oa_ResourcesAdd] as [C] on [A].[ZyId] = [C].id inner join [qp_oa_ResourcesType] as [D] on [C].[ZyLeibie] = [D].id join [qp_oa_ResourcesCangKu] as [B] on [C].[Cangku] = [B].id join [qp_oa_ResourcesQuyu] as [E] on [C].[Quyu] = [E].id   where 1=1";
            string str2 = "select count(A.id) from [qp_oa_ResFp] as [A] inner join [qp_oa_ResourcesAdd] as [C] on [A].[ZyId] = [C].id inner join [qp_oa_ResourcesType] as [D] on [C].[ZyLeibie] = [D].id where 1=1";
