namespace xyoa.Resources.MyRes
{
    ...
    public class MyResApply_SearchListSy : Page
    {
        ...
        {
            decimal num = decimal.Parse(this.KuCun.Text);
            decimal num2 = decimal.Parse(this.Shuliang.Text);
            if (num2 > num)
            {
            ...
            else if (this.ZbRealname.Text != "")
            {
                ...
                string str2 = random.Next(0x989680).ToString();
                ...
                string sql = string.Concat(new object[] { 
                    "insert into qp_oa_ResAppSy  (WpStates,Number,DqBlUsername,DqBlXinming,LcZhuangtai,YjbUser,GlNum,DqNodeId,DqNodeName,YjbNodeId,SNodeId,ZyId,Shuliang,Liyou,Username,Realname,NowTimes) values ('1','", this.Number.Text, "','", this.ZbUsername.Text, "','", this.ZbRealname.Text, "','1','','", glNum, "','", this.FormName.SelectedValue, "','", this.FormName.SelectedItem.Text.Replace("<img src=\"/oaimg/menu/arrow_down.gif\" border=0/>", ""), "','", this.YjbNodeId.Value, ",','", this.YjbNodeId.Value, 
                    "','", this.pulicss.GetFormatStr(this.ZyId.Text), "','", this.pulicss.GetFormatStr(this.Shuliang.Text), "','", this.pulicss.GetFormatStr(this.Liyou.Text), "','", this.Session["username"], "','", this.Session["realname"], "','", DateTime.Now.ToString(), "')"
                 });
                this.List.ExeSql(sql);
                string str4 = "select username,realname from qp_oa_Username where  CHARINDEX(','+username+',','," + this.ZbUsername.Text + ",')   >0 ";
                ...
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='MyResSy.aspx'</script>");
            }
            else
            {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                string str3 = string.Concat(new object[] { "select id,Name from qp_pro_WorkFlow where (CHARINDEX(',", this.Session["BuMenId"], ",',','+ZdBumenId+',') > 0 and States='2') or (CHARINDEX(',", this.Session["username"], ",',','+ZdUsername+',') > 0 and States='3') or (States='1') and FormId='14'" });
