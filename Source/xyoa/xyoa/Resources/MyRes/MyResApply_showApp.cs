namespace xyoa.Resources.MyRes
{
    ...
    public class MyResApply_showApp : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (this.WorkFlowQy.Text == "1")
            {
                ...
                else if (this.ZbRealname.Text != "")
                {
                    ...
                    str3 = string.Concat(new object[] { 
                        "insert into qp_oa_ResAppSy  (FormId,WpStates,Number,DqBlUsername,DqBlXinming,LcZhuangtai,YjbUser,GlNum,DqNodeId,DqNodeName,YjbNodeId,SNodeId,ZyId,Shuliang,Liyou,Username,Realname,NowTimes) values ('", this.FormId.SelectedValue, "','1','", this.Number.Text, "','", this.ZbUsername.Text, "','", this.ZbRealname.Text, "','1','','", str, "','", this.FormName.SelectedValue, "','", this.FormName.SelectedItem.Text.Replace("<img src=\"/oaimg/menu/arrow_down.gif\" border=0/>", ""), "','", this.YjbNodeId.Value, 
                        ",','", this.YjbNodeId.Value, "','", this.pulicss.GetFormatStr(this.ZyId.Text), "','", this.pulicss.GetFormatStr(this.Shuliang.Text), "','", this.pulicss.GetFormatStr(this.Liyou.Text), "','", this.Session["username"], "','", this.Session["realname"], "','", DateTime.Now.ToString(), "')"
                     });
                    ...
                    list.Close();
                    this.pulicss.InsertLog("申请使用物品[" + this.ZyName.Text + "]", "物品申请");
                    base.Response.Write("<script language=javascript>alert('提交成功！');window.parent.location.href='MyResSy.aspx'</script>");
                }
                else
                {
            ...
            else
            {
                ...
                else
                {
                    ...
                    this.List.ExeSql(str3);
                    string str5 = string.Concat(new object[] { "Update qp_oa_ResAppSy  Set  WpStates='2',DqNodeId='',DqBlUsername='已结束',DqBlXinming='已结束',LcZhuangtai='3',YjbUser=YjbUser+'", this.Session["username"], ",',GlNum='", str2, "',YjbNodeId=YjbNodeId+'", this.YjbNodeId.Value, ",',DqNodeName='已结束',SNodeId='", this.YjbNodeId.Value, "' where Number='", this.Number.Text, "'  " });
                    ...
                    string str7 = string.Concat(new object[] { "insert into qp_oa_MyRes  (ZyId,shuliang,Laiyuan,KeyId,Username,Realname,Settime) values ('", this.pulicss.GetFormatStr(this.ZyId.Text), "','", this.Shuliang.Text, "','使用','", base.Request.QueryString["id"], "','", this.Session["Username"], "','", this.Session["Realname"], "','", DateTime.Now.ToString(), "')" });
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                string sql = "select * from qp_pro_WorkFlowQy  where FormId='14'  ";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                ...
                string str4 = string.Concat(new object[] { "select id,Name from qp_pro_WorkFlow where (CHARINDEX(',", this.Session["BuMenId"], ",',','+ZdBumenId+',') > 0 and States='2') or (CHARINDEX(',", this.Session["username"], ",',','+ZdUsername+',') > 0 and States='3') or (States='1') and FormId='14'" });
