namespace xyoa.Resources.MyRes
{
    ...
    public class MyResGmApply_add : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (this.WorkFlowQy.Text == "1")
            {
                ...
                else if (this.ZbRealname.Text != "")
                {
                    ...
                    list.Close();
                    this.pulicss.InsertLog("新增物品购买", "物品购买");
            ...
            else
            {
                ...
                this.List.ExeSql(str3);
                string str5 = string.Concat(new object[] { "Update qp_oa_ResGm  Set  DqNodeId='',DqBlUsername='已结束',DqBlXinming='已结束',LcZhuangtai='3',YjbUser=YjbUser+'", this.Session["username"], ",',GlNum='", str2, "',YjbNodeId=YjbNodeId+'", this.YjbNodeId.Value, ",',DqNodeName='已结束',SNodeId='", this.YjbNodeId.Value, "' where Number='", this.Number.Text, "'  " });
                this.List.ExeSql(str5);
                string str6 = "Update qp_oa_ResourcesAdd    Set KuCun=KuCun+" + this.GmNum.Text + ",AllKuCun=AllKuCun+" + this.GmNum.Text + " where id='" + this.ZyId.Text + "'";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                string sql = "select * from qp_pro_WorkFlowQy  where FormId='11'  ";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                ...
                string str4 = string.Concat(new object[] { "select id,Name from qp_pro_WorkFlow where (CHARINDEX(',", this.Session["BuMenId"], ",',','+ZdBumenId+',') > 0 and States='2') or (CHARINDEX(',", this.Session["username"], ",',','+ZdUsername+',') > 0 and States='3') or (States='1') and FormId='11'" });
