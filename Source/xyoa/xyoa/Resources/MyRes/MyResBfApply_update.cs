namespace xyoa.Resources.MyRes
{
    ...
    public class MyResBfApply_update : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            string sql = string.Concat(new object[] { 
                "Update qp_oa_ResBf  Set  JbName='", this.pulicss.GetFormatStr(this.JbName.Text), "',GmTime='", this.pulicss.GetFormatStr(this.GmTime.Text), "',GmNum='", this.pulicss.GetFormatStr(this.GmNum.Text), "',GmFeiyong='", this.GmFeiyong.Text, "',Gongyinshang='", this.Gongyinshang.Text, "',Shuoming='", this.Shuoming.Text, "'  where id='", int.Parse(base.Request.QueryString["id"]), "'  and Username='", this.Session["username"], 
                "' "
             });
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                string sql = string.Concat(new object[] { "select * from qp_oa_ResBf  where id='", this.pulicss.GetFormatStr(base.Request.QueryString["id"]), "'  and Username='", this.Session["username"], "' " });
