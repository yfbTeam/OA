namespace xyoa.Resources.MyRes
{
    ...
    public class MyResGmApply_update : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            string sql = string.Concat(new object[] { 
                "Update qp_oa_ResGm  Set  JbName='", this.pulicss.GetFormatStr(this.JbName.Text), "',GmTime='", this.pulicss.GetFormatStr(this.GmTime.Text), "',GmNum='", this.pulicss.GetFormatStr(this.GmNum.Text), "',GmFeiyong='", this.GmFeiyong.Text, "',Gongyinshang='", this.Gongyinshang.Text, "',Shuoming='", this.Shuoming.Text, "'  where id='", int.Parse(base.Request.QueryString["id"]), "'  and Username='", this.Session["username"], 
                "' "
             });
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("新增资源购买", "资源购买");
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                string sql = string.Concat(new object[] { "select * from qp_oa_ResGm  where id='", this.pulicss.GetFormatStr(base.Request.QueryString["id"]), "'  and Username='", this.Session["username"], "' " });
