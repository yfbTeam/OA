namespace xyoa.Resources.ResMg
{
    ...
    public class ResMap : Page
    {
        ...
        {
            base.Response.Redirect("ResMap.aspx");
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.shuliang,A.Laiyuan,A.Username,A.Realname,B.Name as CangkuName, E.Name as QuYuName,C.ZyNum, C.ZyName, D.Name as ZyLeibieName from [qp_oa_MyRes] as [A] inner join [qp_oa_ResourcesAdd] as [C] on [A].[ZyId] = [C].id inner join [qp_oa_ResourcesType] as [D] on [C].[ZyLeibie] = [D].id join [qp_oa_ResourcesCangKu] as [B] on [C].[Cangku] = [B].id join [qp_oa_ResourcesQuyu] as [E] on [C].[Quyu] = [E].id where 1=1";
            string str2 = "select count(A.id) from [qp_oa_MyRes] as [A] inner join [qp_oa_ResourcesAdd] as [C] on [A].[ZyId] = [C].id inner join [qp_oa_ResourcesType] as [D] on [C].[ZyLeibie] = [D].id  join [qp_oa_ResourcesCangKu] as [B] on [C].[Cangku] = [B].id join [qp_oa_ResourcesQuyu] as [E] on [C].[Quyu] = [E].id where 1=1";
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "syguihuan")
            {
                ...
                if (list.Read())
                {
                    ...
                    if (reader2.Read())
                    {
                        this.pulicss.InsertLog("归还借用物品" + reader2["ZyName"] + " ", "物品分布列表");
                    }
                    reader2.Close();
            ...
            {
                num = Convert.ToInt32(e.CommandArgument);
                str = "select id,ZyId,KeyId from qp_oa_MyRes where id='" + num + "'";
                list = this.List.GetList(str);
                if (list.Read())
                {
                    ...
                    if (reader2.Read())
                    {
                        this.pulicss.InsertLog("报废物品" + reader2["ZyName"] + " ", "物品分布列表");
                    }
                    reader2.Close();
                    str5 = "  Delete from qp_oa_MyRes  where id='" + num + "' ";
                    this.List.ExeSql(str5);
            ...
            if (e.CommandName == "fbguihuan")
            {
                ...
                if (list.Read())
                {
                    ...
                    if (reader2.Read())
                    {
                        this.pulicss.InsertLog("归还分配物品" + reader2["ZyName"] + " ", "物品分布列表");
                    }
                    reader2.Close();
                    str5 = "  Delete from qp_oa_MyRes  where id='" + num + "' ";
                    this.List.ExeSql(str5);
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (e.Row.Cells[6].Text.ToString() == "借用")
                {
                    ...
                    button.Attributes.Add("onclick", "javascript:return confirm('确认收回[" + e.Row.Cells[1].Text.ToString() + "]吗？')");
                }
                ...
                if (e.Row.Cells[6].Text.ToString() == "分配")
                {
                    ...
                    button3.Attributes.Add("onclick", "javascript:return confirm('确认收回[" + e.Row.Cells[1].Text.ToString() + "]吗？')");
                }
            }
        }
