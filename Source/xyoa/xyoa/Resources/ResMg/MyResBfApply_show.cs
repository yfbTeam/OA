namespace xyoa.Resources.ResMg
{
    ...
    public class MyResBfApply_show : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                string str2;
                str = string.Concat(new object[] { "Update qp_oa_ResBf  Set YjbUser=YjbUser+'", this.Session["username"], ",',DqBlUsername=replace(DqBlUsername,'", this.Session["username"].ToString(), ",','') where id='", int.Parse(base.Request.QueryString["id"]), "'" });
        ...
        protected void Button4_Click(object sender, EventArgs e)
        {
            ...
            {
                string str2;
                str = string.Concat(new object[] { "Update qp_oa_ResBf  Set DqBlUsername=replace(DqBlUsername,'", this.Session["username"].ToString(), ",','') where id='", int.Parse(base.Request.QueryString["id"]), "'" });
                this.List.ExeSql(str);
                if (this.Yijian.Text.Trim() != "")
                {
                    str2 = string.Concat(new object[] { 
            ...
            else
            {
                ...
                this.pulicss.SpUpdateLog(this.Number.Text, this.Yijian.Text, "3", this.GlNum.Text, this.CaoZuo.SelectedValue, DateTime.Now.ToString());
                str = string.Concat(new object[] { "Update qp_oa_ResBf  Set  DqNodeId='',DqBlUsername='已结束',DqBlXinming='已结束',LcZhuangtai='3',YjbUser=YjbUser+'", this.Session["username"], ",',GlNum='", str3, "',YjbNodeId=YjbNodeId+'", this.YjbNodeId.Value, ",',DqNodeName='已结束',SNodeId='", this.YjbNodeId.Value, "' where id='", int.Parse(base.Request.QueryString["id"]), "'  " });
                this.List.ExeSql(str);
                string sql = "Update qp_oa_ResourcesAdd    Set KuCun=KuCun-" + this.GmNum.Text + ",BfKuCun=BfKuCun+" + this.GmNum.Text + " where id='" + this.ZyId.Text + "'";
                this.List.ExeSql(sql);
                this.pulicss.InsertMessage("您的物品报废已经审批完成，对应物品[" + this.ZyName.Text + "]", this.ZbUsername.Text, this.ZbRealname.Text, "/Resources/MyRes/MyResBfApply.aspx");
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='MyResBfApply.aspx'</script>");
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                string sql = "Update qp_oa_ResBf  Set  LcZhuangtai='2' where id='" + int.Parse(base.Request.QueryString["id"]) + "'  ";
                this.List.ExeSql(sql);
                string str3 = "select * from qp_oa_ResBf  where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "' ";
        ...
        {
            int num = 0;
            string sql = "select DqBlUsername from qp_oa_ResBf   where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'";
            SqlDataReader list = this.List.GetList(sql);
            if (list.Read())
            {
