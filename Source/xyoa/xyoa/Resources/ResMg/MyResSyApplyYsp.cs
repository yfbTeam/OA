namespace xyoa.Resources.ResMg
{
    ...
    public class MyResSyApplyYsp : Page
    {
        ...
        {
            base.Response.Redirect("MyResSyApplyYsp.aspx");
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.*, C.ZyName, C.ZyNum from [qp_oa_ResAppSy] as [A] join [qp_oa_ResourcesAdd] as [C] on [A].[ZyId] = [C].id where CHARINDEX('," + this.Session["username"] + ",',','+YjbUser+',')   >0";
            string str2 = "select count(A.id) from [qp_oa_ResAppSy] as [A] join [qp_oa_ResourcesAdd] as [C] on [A].[ZyId] = [C].id where CHARINDEX('," + this.Session["username"] + ",',','+YjbUser+',')   >0";
