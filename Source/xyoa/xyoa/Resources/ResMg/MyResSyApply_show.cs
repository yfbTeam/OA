namespace xyoa.Resources.ResMg
{
    ...
    public class MyResSyApply_show : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                string str2;
                str = string.Concat(new object[] { "Update qp_oa_ResAppSy  Set YjbUser=YjbUser+'", this.Session["username"], ",',DqBlUsername=replace(DqBlUsername,'", this.Session["username"].ToString(), ",','') where id='", int.Parse(base.Request.QueryString["id"]), "'" });
                ...
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='MyResSyApply.aspx'</script>");
            }
            else if (this.ZbRealname.Text != "")
            {
                ...
                while (list.Read())
                {
                    this.pulicss.InsertMessage("您有物品使用需要审批，对应物品[" + this.ZyName.Text + "]", list["username"].ToString(), list["realname"].ToString(), "/Resources/ResMg/MyResSyApply.aspx");
                }
                list.Close();
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='MyResSyApply.aspx'</script>");
            }
        ...
        {
            base.Response.Redirect("MyResSyApply.aspx");
        }
        ...
        protected void Button4_Click(object sender, EventArgs e)
        {
            ...
            {
                string str2;
                str = string.Concat(new object[] { "Update qp_oa_ResAppSy  Set DqBlUsername=replace(DqBlUsername,'", this.Session["username"].ToString(), ",','') where id='", int.Parse(base.Request.QueryString["id"]), "'" });
                this.List.ExeSql(str);
                if (this.Yijian.Text.Trim() != "")
                {
                    str2 = string.Concat(new object[] { 
            ...
            else
            {
                ...
                this.pulicss.SpUpdateLog(this.Number.Text, this.Yijian.Text, "3", this.GlNum.Text, this.CaoZuo.SelectedValue, DateTime.Now.ToString());
                str = string.Concat(new object[] { "Update qp_oa_ResAppSy  Set  WpStates='2',DqNodeId='',DqBlUsername='已结束',DqBlXinming='已结束',LcZhuangtai='3',YjbUser=YjbUser+'", this.Session["username"], ",',GlNum='", str3, "',YjbNodeId=YjbNodeId+'", this.YjbNodeId.Value, ",',DqNodeName='已结束',SNodeId='", this.YjbNodeId.Value, "' where id='", int.Parse(base.Request.QueryString["id"]), "'  " });
                this.List.ExeSql(str);
                string sql = "Update qp_oa_ResourcesAdd    Set KuCun=KuCun-" + this.Shuliang.Text + " where id='" + this.ZyId.Text + "'";
                this.List.ExeSql(sql);
                string str5 = "insert into qp_oa_MyRes  (ZyId,shuliang,Laiyuan,KeyId,Username,Realname,Settime) values ('" + this.pulicss.GetFormatStr(this.ZyId.Text) + "','" + this.Shuliang.Text + "','使用','" + base.Request.QueryString["id"] + "','" + this.Username.Text + "','" + this.Realname.Text + "','" + DateTime.Now.ToString() + "')";
                this.List.ExeSql(str5);
                this.pulicss.InsertMessage("您的物品使用已经审批完成，对应物品[" + this.ZyName.Text + "]", this.ZbUsername.Text, this.ZbRealname.Text, "/Resources/MyRes/MyResSy.aspx");
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='MyResSyApply.aspx'</script>");
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                string sql = "Update qp_oa_ResAppSy  Set  LcZhuangtai='2' where id='" + int.Parse(base.Request.QueryString["id"]) + "'  ";
                this.List.ExeSql(sql);
                string str3 = "select * from qp_oa_ResAppSy  where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "' ";
        ...
        {
            int num = 0;
            string sql = "select DqBlUsername from qp_oa_ResAppSy   where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'";
            SqlDataReader list = this.List.GetList(sql);
            if (list.Read())
            {
