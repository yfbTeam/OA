namespace xyoa.MyWork.MySet
{
    ...
    public class MokuaiKz_show : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                list.Close();
                string str4 = null;
                string str5 = "select [keyid] from [qp_oa_MyUrl] where Username='" + this.Session["username"] + "' and leixing='2'";
                ...
                string str6 = string.Concat(new object[] { "select * from qp_oa_OtherMenu where ((CHARINDEX(',", this.Session["BuMenId"], ",',','+ZdBumenId+',') > 0 and States='2') or (CHARINDEX(',", this.Session["username"], ",',','+ZdUsername+',') > 0 and States='3') or (States='1')) and id not in (", str4, ")  order by Paixun asc" });
                ...
                while (reader3.Read())
                {
                    ...
                    (bag = this.ViewState)["myurl"] = string.Concat(new object[] { 
                        obj2, "<li onclick=\"addWin('kzkeyid", reader3["id"], "5322','", reader3["id"], "','", reader3["name"], "','", reader3["urlname"], "','0','0','", reader3["pic"], "')\" id=\"kzkeyid", reader3["id"], "\"><img src=\"/Images/desktopIOC/", reader3["pic"], "\"/><br />", 
                        reader3["name"], "</li>"
                     });
                }
                reader3.Close();
