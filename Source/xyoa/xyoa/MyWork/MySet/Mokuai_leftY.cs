namespace xyoa.MyWork.MySet
{
    ...
    public class Mokuai_leftY : Page
    {
        ...
        private void BindTree(TreeNodeCollection Nds)
        {
            ...
            while (list.Read())
            {
                ...
                {
                    child.Text = " <a href='MokuaiKz_showY.aspx?menhu=" + base.Request.QueryString["menhu"] + "' target='nextrform'>" + list["urlname"].ToString() + "</a>";
                }
                else if (list["leixing"].ToString() == "12")
                {
                    child.Text = " <a href='MokuaiMh_showY.aspx?menhu=" + base.Request.QueryString["menhu"] + "' target='nextrform'>" + list["urlname"].ToString() + "</a>";
                }
                else
                {
                    child.Text = string.Concat(new object[] { " <a href='Mokuai_showY.aspx?id=", num, "&menhu=", base.Request.QueryString["menhu"], "' target='nextrform'>", list["urlname"].ToString(), "</a>" });
                }
