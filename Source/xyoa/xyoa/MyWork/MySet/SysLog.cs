namespace xyoa.MyWork.MySet
{
    ...
    public class SysLog : Page
    {
        ...
        {
            this.pulicss.QuanXianVis(this.Button3, "hhhh9le", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.DelData, "hhhh9le", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.GridView1, "hhhh9lc", this.Session["perstr"].ToString());
        }
        ...
        {
            base.Response.Redirect("SysLog.aspx");
        }

        protected void Button3_Click(object sender, EventArgs e)
        {
            string sql = "  Delete from qp_oa_SystemLog  where username='" + this.Session["username"] + "'";
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select * from qp_oa_SystemLog where Username='" + this.Session["username"] + "'";
            string str2 = "select count(id) from qp_oa_SystemLog where Username='" + this.Session["username"] + "'";
        ...
        protected void DelData_Click(object sender, EventArgs e)
        {
            ...
            string sql = string.Concat(new object[] { "  Delete from qp_oa_SystemLog  where ID in (", str, ") and username='", this.Session["username"], "'" });
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("删除系统日志", "系统日志");
            this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
        }
