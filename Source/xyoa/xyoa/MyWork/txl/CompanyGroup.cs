namespace xyoa.MyWork.txl
{
    ...
    public class CompanyGroup : Page
    {
        ...
        {
            base.Response.Redirect("CompanyGroup_add.aspx");
        }
        ...
        {
            this.pulicss.QuanXianVis(this.AddData, "hhhh5aa", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.UpdateData, "hhhh5ac", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.ShowData, "hhhh5ab", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.DelData, "hhhh5ad", this.Session["perstr"].ToString());
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.MoveTel,A.Name, B.[Name] as BuMenName,  D.[Name] as ZhiweiName from [qp_oa_CompanyGroup] as [A] inner join [qp_oa_Bumen] as [B] on [A].[BuMenId] = [B].[Id] inner join [qp_oa_Zhiwei] as [D] on [A].[Zhiweiid] = [D].[Id] where 1=1";
            string str2 = "select count(id) from qp_oa_CompanyGroup as [A] where 1=1";
        ...
        protected void DelData_Click(object sender, EventArgs e)
        {
            ...
            string sql = "  Delete from qp_oa_CompanyGroup  where ID in (" + str + ")";
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("删除单位通讯录[" + this.CheckCbxNameDel() + "]", "单位通讯录");
            this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
        }
        ...
        {
            base.Response.Redirect("CompanyGroup_show.aspx?id=" + this.CheckCbxUpdate() + "");
        }

        protected void UpdateData_Click(object sender, EventArgs e)
        {
            base.Response.Redirect("CompanyGroup_update.aspx?id=" + this.CheckCbxUpdate() + "");
        }
    }
}
