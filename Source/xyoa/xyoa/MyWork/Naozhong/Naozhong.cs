namespace xyoa.MyWork.Naozhong
{
    ...
    public class Naozhong : Page
    {
        ...
        {
            base.Response.Redirect("Naozhong_add.aspx");
        }
        ...
        {
            this.pulicss.QuanXianVis(this.AddData, "hhhh1", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.UpdateData, "hhhh1", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.DelData, "hhhh1", this.Session["perstr"].ToString());
        }
        ...
        public string CreateMidSql()
        {
            ...
            {
                str = str + " and convert(char(10),cast(txtime as datetime),120)=convert(char(10),cast('" + this.pulicss.GetFormatStr(this.Nowtimes.Text) + "' as datetime),120) ";
            }
            if (this.Types.SelectedValue != "")
            {
                str = str + " and Types = '" + this.Types.SelectedValue + "'";
            }
            return str;
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.*, B.[CodeName] as cTypes from [qp_oa_Naozhong] as [A] inner join [SystemCode] as [B] on [A].[Types] = [B].[CodeId]  and [B].FieldName = '5' where username='" + this.Session["username"] + "'";
            string str2 = "select count(id) from qp_oa_Naozhong where username='" + this.Session["username"] + "'";
        ...
        protected void DelData_Click(object sender, EventArgs e)
        {
            ...
            string sql = "  Delete from qp_oa_Naozhong  where ID in (" + str + ")";
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("删除电子闹钟", "电子闹钟");
            this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
        }
        ...
        {
            base.Response.Redirect("Naozhong_update.aspx?id=" + this.CheckCbxUpdate() + "");
        }
    }
}
