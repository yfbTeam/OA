namespace xyoa.MyWork.Metting
{
    ...
    public class MynetMettingYcy : Page
    {
        ...
        {
            base.Response.Redirect("MynetMettingYcy.aspx");
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = string.Concat(new object[] { "select A.id,A.Name,A.MettingIp,A.Starttime,A.Endtime,A.State,A.MettingHeader,A.MettingHeaderUser, C.CodeName from [qp_oa_NetMetting] as [A] join [SystemCode] as [C] on [A].[State] = [C].CodeId and [C].FieldName = '8' where CHARINDEX(',", this.Session["Username"], ",',','+NbPeopleUser+',')   >   0  and CHARINDEX(',", this.Session["Username"], ",',','+CjUsername+',')   >   0 " });
            string str2 = string.Concat(new object[] { "select count(id) from qp_oa_NetMetting where  CHARINDEX(',", this.Session["Username"], ",',','+NbPeopleUser+',')   >   0 and CHARINDEX(',", this.Session["Username"], ",',','+CjUsername+',')   >   0 " });
