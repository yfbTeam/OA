namespace xyoa.MyWork.Metting
{
    ...
    public class MyMettingWcy : Page
    {
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = string.Concat(new object[] { "select A.* from [qp_oa_MettingApply] as [A]  where  (State='2' or State='3'  or State='4' or State='5' ) and   CHARINDEX(',", this.Session["Username"], ",',','+NbPeopleUser+',')   >   0  and CHARINDEX(',", this.Session["Username"], ",',','+CjUsername+',')   =   0" });
            string str2 = string.Concat(new object[] { "select count(id) from qp_oa_MettingApply where  (State='2' or State='3'  or State='4' or State='5' ) and   CHARINDEX(',", this.Session["Username"], ",',','+NbPeopleUser+',')   >   0 and CHARINDEX(',", this.Session["Username"], ",',','+CjUsername+',')   =   0" });
