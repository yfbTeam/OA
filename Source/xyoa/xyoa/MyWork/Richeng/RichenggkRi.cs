namespace xyoa.MyWork.Richeng
{
    ...
    public class RichenggkRi : Page
    {
        ...
        public void BindList()
        {
            ...
            string str2 = null;
            string sql = "select id,titles,StartTime,Realname,username,EndTime from qp_oa_MyRicheng where    Gongkai='是' and (('" + str + "' between StartTime and  EndTime) or (convert(char(10),cast(StartTime as datetime),120)=convert(char(10),cast('" + str + "' as datetime),120)) or (convert(char(10),cast(EndTime as datetime),120)=convert(char(10),cast('" + str + "' as datetime),120)) ) and (datediff(dd,StartTime,'" + str + "')!=0 or datediff(dd,EndTime,'" + str + "')!=0)";
            SqlDataReader list = this.List.GetList(sql);
            while (list.Read())
            {
                text = str2;
                str2 = string.Concat(new object[] { text, "<div class=Celldiv>", list["StartTime"].ToString(), "&nbsp;&nbsp;-&nbsp;&nbsp;", list["EndTime"].ToString(), "<img src=\"/oaimg/r/1.gif\" border=0 hspace=2><a href='javascript:void(0)' onclick='senduser(\"", list["username"], "\")'><font color=#000000>", list["realname"], "：</font></a><a href=Richenggk_show.aspx?id=", list["id"].ToString(), "> <font color=blue>", list["titles"].ToString(), "</font></a><br></div>" });
            }
            list.Close();
