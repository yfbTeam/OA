namespace xyoa.MyWork.Richeng
{
    ...
    public class RichengXbYue : Page
    {
        public int a;
        public int b;
        public int c;
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (str != null)
                    {
                        ...
                        string sql = string.Concat(new object[] { "select StartTime,EndTime,id,titles,realname from qp_oa_MyRicheng where (CHARINDEX(',", this.Session["username"], ",',','+XbUsername+',') > 0) and datediff(dd,StartTime,'", e.Day.Date.ToShortDateString(), "')=0" });
                        SqlDataReader list = this.List.GetList(sql);
                        while (list.Read())
                        {
                            TableCell cell1 = e.Cell;
                            object text = cell1.Text;
                            cell1.Text = string.Concat(new object[] { text, "<img src=\"/oaimg/r/1.gif\" border=0 hspace=2><font color=#666666>", list["StartTime"].ToString(), "&nbsp;&nbsp;-&nbsp;&nbsp;", list["EndTime"].ToString(), "</font><a href=RichengXb_show.aspx?id=", list["id"].ToString(), "><font color=#000000>", list["realname"], "：</font><font color=blue>", list["titles"].ToString(), "</font></a><br><br>" });
                        }
                        list.Close();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string sql = "select StartTime from qp_oa_MyRicheng where (CHARINDEX('," + this.Session["username"] + ",',','+XbUsername+',') > 0)";
                SqlDataReader list = this.List.GetList(sql);
                while (list.Read())
                {
