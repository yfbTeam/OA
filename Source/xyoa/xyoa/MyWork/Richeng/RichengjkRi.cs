namespace xyoa.MyWork.Richeng
{
    ...
    public class RichengjkRi : Page
    {
        ...
        public void BindList()
        {
            ...
            string str2 = null;
            string sql = string.Concat(new object[] { "select id,titles,StartTime,Realname,username,EndTime from qp_oa_MyRicheng where    CHARINDEX(','+Username+',',','+(select Glusername from qp_oa_Richengsz where Username='", this.Session["username"], "')+',') > 0 and (('", str, "' between StartTime and  EndTime) or (convert(char(10),cast(StartTime as datetime),120)=convert(char(10),cast('", str, "' as datetime),120)) or (convert(char(10),cast(EndTime as datetime),120)=convert(char(10),cast('", str, "' as datetime),120)) ) and (datediff(dd,StartTime,'", str, "')!=0 or datediff(dd,EndTime,'", str, "')!=0)" });
            ...
            for (int i = 0; i < 0x18; i++)
            {
                ...
                string str5 = null;
                string str6 = string.Concat(new object[] { "select id,titles,StartTime,Realname,username,EndTime from qp_oa_MyRicheng  where   CHARINDEX(','+Username+',',','+(select Glusername from qp_oa_Richengsz where Username='", this.Session["username"], "')+',') > 0 and datediff(hh,StartTime,'", str4, "')=0 and datediff(hh,EndTime,'", str4, "')=0" });
