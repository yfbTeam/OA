    ...
    public class Richengmy : Page
    {
        ...
        protected void Calendar1_DayRender(object sender, DayRenderEventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (str != null)
                    {
                        e.Cell.Text = "<table width=100% height=100 border=0 cellpadding=0 cellspacing=0><tr><td height=20 align=right bgcolor=#DEE7F3>" + e.Day.Date.Day.ToString() + "<a href=RichengmyList_add.aspx?day=" + e.Day.Date.ToShortDateString() + " class=LinkLine><img src=\"/oaimg/12task.png\" border=0  hspace=2></a></td></tr><tr><td height=80 valign=top>";
                        string sql = string.Concat(new object[] { "select StartTime,EndTime,id,titles from qp_oa_MyRicheng where username='", this.Session["username"], "' and datediff(dd,StartTime,'", e.Day.Date.ToShortDateString(), "')=0" });
                        SqlDataReader list = this.List.GetList(sql);
                        while (list.Read())
                        {
                            TableCell cell1 = e.Cell;
                            string text = cell1.Text;
                            cell1.Text = text + "<img src=\"/oaimg/r/1.gif\" border=0 hspace=2><font color=#666666>" + list["StartTime"].ToString() + "&nbsp;&nbsp;-&nbsp;&nbsp;" + list["EndTime"].ToString() + "</font><a href=RichengmyList_show.aspx?id=" + list["id"].ToString() + "><font color=blue>" + list["titles"].ToString() + "</font></a><br><br>";
                        }
                        list.Close();
                    ...
                    {
                        e.Cell.Text = "<table width=100% height=100 border=0 cellpadding=0 cellspacing=0><tr><td height=20 align=right bgcolor=#DEE7F3>" + e.Day.Date.Day.ToString() + "<a href=RichengmyList_add.aspx?day=" + e.Day.Date.ToShortDateString() + " class=LinkLine  title='点击新增日程'><img src=\"/oaimg/12task.png\" border=0 hspace=2></a></td></tr><tr><td height=80 valign=top>&nbsp;</td></tr></table>";
                    }
                }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string sql = "select StartTime from qp_oa_MyRicheng where username='" + this.Session["username"] + "'";
                SqlDataReader list = this.List.GetList(sql);
                while (list.Read())
                {
