namespace xyoa.JqMain
{
    ...
    public class Mymenhu : Page
    {
        ...
        public void BindList(string leixing)
        {
            ...
            string sql = "select   * from qp_oa_AllUrl where (CHARINDEX(''+quanxian+'','" + str + "')   >   0 ) and id in (" + leixing + ") and menhulm='1' and ParentNodesID!='0' and ifopen='1' order by paixu asc";
            ...
            while (list.Read())
            {
                ...
                {
                    str3 = "select top 4 * from [qp_oa_MettingApply] as [A]  where CHARINDEX('," + this.Session["username"] + ",',','+DqBlUsername+',')   >0  and (LcZhuangtai='1' or LcZhuangtai='2')  order by A.id desc";
                    reader2 = this.List.GetList(str3);
                    str4 = str4 + "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"4\">";
                    while (reader2.Read())
                    {
                ...
                {
                    str3 = string.Concat(new object[] { "select top 4 A.id,A.Titles,A.TxTime,A.Username,A.TxRealname, C.Name from [qp_oa_YjBox] as [A] join [qp_oa_YjBoxSz] as [C] on [A].[BoxId] = [C].id and CHARINDEX('", this.Session["username"], ",',C.Username) > 0 where  CHARINDEX(',", this.Session["username"], ",',','+YdUsername+',')   =0 order by A.id desc" });
                    reader2 = this.List.GetList(str3);
                    str4 = str4 + "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"4\">";
                    while (reader2.Read())
                    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!base.IsPostBack)
            {
                string str2;
                SqlDataReader list;
                if (this.pulicss.GetFormatStr(base.Request.QueryString["id"]) == "0")
                {
                    string leixing = null;
                    str2 = string.Concat(new object[] { "select Lmid from qp_oa_Menhu  where (CHARINDEX(',", this.Session["BuMenId"], ",',','+ZdBumenId+',') > 0 and States='2') or (CHARINDEX(',", this.Session["username"], ",',','+ZdUsername+',') > 0 and States='3') or (States='1') order by Paixun asc" });
                    list = this.List.GetList(str2);
                    while (list.Read())
                    {
                        leixing = leixing + "" + list["Lmid"].ToString() + ",";
                    }
                    list.Close();
                    leixing = leixing + "0";
                ...
                {
                    str2 = "select * from qp_oa_Menhu where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'";
                    list = this.List.GetList(str2);
                    if (list.Read())
                    {
                        this.BindList(list["Lmid"].ToString());
                    }
                    list.Close();
                }
            }
