namespace xyoa.PublicWork.ComThingMg
{
    ...
    public class ComThingMg_update : Page
    {
        ...
        {
            this.pulicss.QuanXianBack("jjjj2bc", this.Session["perstr"].ToString());
        ...
        {
            string sql = string.Concat(new object[] { "Update qp_oa_ComThingMg  Set  titles='", this.pulicss.GetFormatStr(this.titles.Text), "',Content='", this.pulicss.GetFormatStrbjq(this.Content.Value), "' where id='", int.Parse(base.Request.QueryString["id"]), "'  " });
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("修改大事记", "大事记");
            base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='ComThingMg.aspx'</script>");
        }

        protected void Button2_Click(object sender, EventArgs e)
        {
            base.Response.Redirect("ComThingMg.aspx");
        }

        protected void Button4_Click(object sender, EventArgs e)
        {
            string str = base.Server.MapPath("dashiji/");
            ...
            if (this.uploadFile.PostedFile.ContentLength != 0)
            {
                ...
                else
                {
                    ...
                    this.pulicss.Insertfile(fileName, "PublicWork/ComThingMg/dashiji/" + str2 + "", this.Number.Text, filetype);
                    this.pulicss.InsertLog("上传附件[" + fileName + "]", "大事记");
                    this.BindFjlbList();
                }
            }
        }
