namespace xyoa.PublicWork.Hetong
{
    ...
    public class MyHeTong_add : Page
    {
            ...
            if (this.WorkFlowQy.Text == "1")
            {
                ...
                else if (this.ZbRealname.Text != "")
                {
                    ...
                    str3 = string.Concat(new object[] { 
                        "insert into qp_oa_HeTong (FormId,Number,DqBlUsername,DqBlXinming,LcZhuangtai,YjbUser,GlNum,DqNodeId,DqNodeName,YjbNodeId,SNodeId,Zhuti,Neirong,Hetonghao,Jine,Danwei,Fenlei,Zhuangtai,Qianyue,Qixian,Daoqi,Username,Realname,NowTimes) values ('", this.FormId.SelectedValue, "','", this.pulicss.GetFormatStr(this.Number.Text), "','", this.ZbUsername.Text, "','", this.ZbRealname.Text, "','1','','", str, "','", this.FormName.SelectedValue, "','", this.FormName.SelectedItem.Text.Replace("<img src=\"/oaimg/menu/arrow_down.gif\" border=0/>", ""), "','", this.YjbNodeId.Value, 
                        ",','", this.YjbNodeId.Value, "','", this.pulicss.GetFormatStr(this.Zhuti.Text), "','", this.pulicss.GetFormatStrbjq(this.Neirong.Value), "','", this.pulicss.GetFormatStr(this.Hetonghao.Text), "','", this.pulicss.GetFormatStr(this.Jine.Text), "','", this.pulicss.GetFormatStr(this.Danwei.Text), "','", this.Fenlei.SelectedValue, "','", this.Zhuangtai.SelectedValue, 
                        "','", this.pulicss.GetFormatStr(this.Qianyue.Text), "','", this.Qixian.SelectedValue, "','", this.pulicss.GetFormatStr(this.Daoqi.Text), "','", this.Session["Username"], "','", this.Session["Realname"], "','", DateTime.Now.ToShortDateString(), "')"
                     });
                    ...
                    list.Close();
                    this.pulicss.InsertLog("新增合同[" + this.Zhuti.Text + "]", "我的合同");
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    string sql = "select * from qp_pro_WorkFlowQy  where FormId='7'  ";
                    SqlDataReader list = this.List.GetList(sql);
                    if (list.Read())
                    {
                    ...
                    this.BindDroList();
                    this.BindAttribute();
                    this.Qianyue.Text = DateTime.Now.ToShortDateString();
                    Random random = new Random();
                    ...
                    string str4 = string.Concat(new object[] { "select id,Name from qp_pro_WorkFlow where (CHARINDEX(',", this.Session["BuMenId"], ",',','+ZdBumenId+',') > 0 and States='2') or (CHARINDEX(',", this.Session["username"], ",',','+ZdUsername+',') > 0 and States='3') or (States='1') and FormId='7'" });
