namespace xyoa.PublicWork.Hetong
{
    ...
    public class HetongySp : Page
    {
        ...
        {
            this.pulicss.QuanXianVis(this.GridView1, "jjjj8b", this.Session["perstr"].ToString());
        }
        ...
        {
            base.Response.Redirect("HetongySp.aspx");
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str2 = ("select A.Username as AUsername,A.Realname as ARealname,A.id,A.LcZhuangtai,A.DqBlXinming,A.DqBlUsername,A.Zhuti,A.Hetonghao,A.Fenlei,A.Zhuangtai,A.Danwei,A.Jine,B.Name as FenleiName from [qp_oa_HeTong] as [A] inner join [qp_oa_HetongLb] as [B] on [A].[Fenlei] = [B].id where CHARINDEX('," + this.Session["username"] + ",',','+YjbUser+',')   >0 ") + SqlCreate;
            string sql = string.Concat(new object[] { "select sum(A.Jine) as AllShuliang from  [qp_oa_HeTong] as [A] inner join [qp_oa_HetongLb] as [B] on [A].[Fenlei] = [B].id where CHARINDEX(',", this.Session["username"], ",',','+YjbUser+',')   >0  ", SqlCreate, " " });
            ...
            string str6 = ("select count(A.id) from [qp_oa_HeTong] as [A] inner join [qp_oa_HetongLb] as [B] on [A].[Fenlei] = [B].id where CHARINDEX('," + this.Session["username"] + ",',','+YjbUser+',')   >0 ") + SqlCreate;
