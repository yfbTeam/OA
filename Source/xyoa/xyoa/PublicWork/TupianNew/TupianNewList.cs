namespace xyoa.PublicWork.TupianNew
{
    ...
    public class TupianNewList : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                string sql = null;
                sql = string.Concat(new object[] { "select id,Titles,photo from qp_oa_TupianNew where (CHARINDEX(',", this.Session["BuMenId"], ",',','+ZdBumenId+',') > 0 and States='2') or (CHARINDEX(',", this.Session["username"], ",',','+ZdUsername+',') > 0 and States='3') or (States='1') order by settime desc" });
