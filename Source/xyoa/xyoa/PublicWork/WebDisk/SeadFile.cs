namespace xyoa.PublicWork.WebDisk
{
    ...
    public class SeadFile : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='MyWebDisk_show.aspx?id=0'</script>");
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                string sql = "select QxString from qp_oa_username where Username='" + this.Session["username"] + "'";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    this.ViewState["QxStringStr"] = list["QxString"].ToString();
                }
                list.Close();
                if (!this.Page.IsPostBack)
                {
                    ...
                    string str2 = "select A.[OldName],A.[NewName],A.[Fjtype],A.[id] from [qp_oa_MyWebDiskFile] as [A] where A.id in (" + base.Request.QueryString["id"] + ")";
                    SqlDataReader reader2 = this.List.GetList(str2);
                    while (reader2.Read())
                    {
                        this.pulicss.Insertfile(reader2["OldName"].ToString(), reader2["NewName"].ToString(), this.Number.Text, reader2["Fjtype"].ToString());
                        string str3 = string.Concat(new object[] { "insert into qp_oa_MyWebDiskLog  (KeyId,Username,Realname,SetTime,Contents,BuMenId) values ('", reader2["id"].ToString(), "','", this.Session["username"], "','", this.Session["realname"], "','", DateTime.Now.ToString(), "','转发文件','", this.Session["BuMenId"], "')" });
