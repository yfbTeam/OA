namespace xyoa.PublicWork.WebDisk
{
    ...
    public class WebDiskLx_qx_add : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                string sql = "select nameid from qp_oa_WebDiskLxQx  where KeyId='" + this.pulicss.GetFormatStr(base.Request.QueryString["KeyId"]) + "' and Types='1' ";
                SqlDataReader list = this.List.GetList(sql);
                while (list.Read())
                {
                ...
                list.Close();
                str = str + "0";
                string sQL = "select * from qp_oa_WebDiskLxGL  where id not in (" + str + ")";
                this.SourceListBox.Items.Clear();
