namespace xyoa.PublicWork.Zhidu
{
    ...
    public class Zhidu_update : Page
    {
        ...
        {
            this.pulicss.QuanXianBack("jjjj6bc", this.Session["perstr"].ToString());
        ...
        {
            string sql = string.Concat(new object[] { "Update qp_oa_ZhiduMg  Set  TypeId='", this.TypeId.SelectedValue, "',Titles='", this.pulicss.GetFormatStr(this.Titles.Text), "',Content='", this.pulicss.GetFormatStrbjq(this.Content.Value), "',Settime='", DateTime.Now.ToString(), "'  where id='", int.Parse(base.Request.QueryString["id"]), "'  " });
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("修改规章制度", "规章制度");
            base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='Zhidu_show.aspx?id=" + this.TypeId.SelectedValue + "'</script>");
        }

        protected void Button2_Click(object sender, EventArgs e)
        {
            base.Response.Redirect("Zhidu_show.aspx?id=" + base.Request.QueryString["typeid"] + "");
        }

        protected void Button4_Click(object sender, EventArgs e)
        {
            string str = base.Server.MapPath("Zhidu/");
            ...
            if (this.uploadFile.PostedFile.ContentLength != 0)
            {
                ...
                else
                {
                    ...
                    this.pulicss.Insertfile(fileName, "PublicWork/Zhidu/Zhidu/" + str2 + "", this.Number.Text, filetype);
                    this.pulicss.InsertLog("上传附件[" + fileName + "]", "规章制度");
                    this.BindFjlbList();
                }
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    string sQL = "select id,name  from qp_oa_ZhiduLx order by Paixun asc";
