namespace xyoa.PublicWork.BumenNewsMg
{
    ...
    public class BumenNewsSc : Page
    {
        ...
        {
            this.pulicss.QuanXianVis(this.GridView1, "jjjj3c", this.Session["perstr"].ToString());
        }
        ...
        {
            base.Response.Redirect("BumenNewsSc.aspx");
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = string.Concat(new object[] { "select id,titles,Settime,Username,Realname,FqBumen,YdUsername from qp_oa_BumenNewsMg where ((CHARINDEX(',", this.Session["BuMenId"], ",',','+ZdBumenId+',') > 0 and States='2') or (CHARINDEX(',", this.Session["username"], ",',','+ZdUsername+',') > 0 and States='3') or (States='1')) and CHARINDEX(',", this.Session["username"], ",',','+ScUsername+',')   >0" });
            string str2 = string.Concat(new object[] { "select count(id) from qp_oa_BumenNewsMg where  ((CHARINDEX(',", this.Session["BuMenId"], ",',','+ZdBumenId+',') > 0 and States='2') or (CHARINDEX(',", this.Session["username"], ",',','+ZdUsername+',') > 0 and States='3') or (States='1')) and CHARINDEX(',", this.Session["username"], ",',','+ScUsername+',')   >0" });
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "shoucan")
            {
                ...
                string sql = string.Concat(new object[] { "Update qp_oa_BumenNewsMg  Set ScUsername=replace(ScUsername,'", this.Session["username"].ToString(), ",','')  where ID='", num, "'" });
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                Label label = (Label) e.Row.FindControl("YdUsername");
                Label label2 = (Label) e.Row.FindControl("Label2");
                string str = "," + label.Text + ",";
                string str2 = "," + this.Session["username"].ToString() + ",";
                if (str.IndexOf(str2) < 0)
                {
                    label2.Text = "<font color=red>未阅读</font>";
                }
                else
                {
                    label2.Text = "已阅读";
        ...
        {
            base.Response.Redirect("BumenNewsSc_show.aspx?id=" + this.CheckCbxUpdate() + "");
        }
    }
}
