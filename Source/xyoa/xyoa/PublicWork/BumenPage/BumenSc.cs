namespace xyoa.PublicWork.BumenPage
{
    ...
    public class BumenSc : Page
    {
        ...
        {
            this.pulicss.QuanXianVis(this.GridView1, "jjjja2e", this.Session["perstr"].ToString());
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.ZhuyeId,A.titles,A.Settime,A.Username,A.Realname,B.[Name] as BuMenName,D.[Leixing] as LeixingName from [qp_oa_BumenWz] as [A] inner join [qp_oa_BumenZy] as [C] on [A].[ZhuyeId] = [C].[Id] inner join [qp_oa_Bumen] as [B] on [C].[BuMenId] = [B].[Id] inner join [qp_oa_BumenWzLb] as [D] on [A].[WzLeibie] = [D].[Id]  where  CHARINDEX('," + this.Session["username"] + ",',','+A.ScUsername+',')   >0";
            string str2 = "select count(A.id) from [qp_oa_BumenWz] as [A] inner join [qp_oa_BumenZy] as [C] on [A].[ZhuyeId] = [C].[Id] inner join [qp_oa_Bumen] as [B] on [C].[BuMenId] = [B].[Id] inner join [qp_oa_BumenWzLb] as [D] on [A].[WzLeibie] = [D].[Id]  where  CHARINDEX('," + this.Session["username"] + ",',','+A.ScUsername+',')   >0";
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "shoucan")
            {
                ...
                string sql = string.Concat(new object[] { "Update qp_oa_BumenWz  Set ScUsername=replace(ScUsername,'", this.Session["username"].ToString(), ",','')  where ID='", num, "'" });
        ...
        {
            base.Response.Redirect("BumenSc_show.aspx?id=" + this.CheckCbxUpdate() + "");
        }
    }
}
