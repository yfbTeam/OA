namespace xyoa.PublicWork.Tupian
{
    ...
    public class TupianLxList_show : Page
    {
        ...
        {
            base.Response.Redirect("TupianLxList_show.aspx?id=" + base.Request.QueryString["id"] + "");
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            if (base.Request.QueryString["id"].ToString() == "0")
            {
                ...
                str2 = string.Concat(new object[] { "select count(A.id) from [qp_oa_TupianMg] as [A] join [qp_oa_TupianLx] as [C] on [A].[TypeId] = [C].id  and   ((CHARINDEX(',", this.Session["BuMenId"], ",',','+[C].ZdBumenId+',') > 0 and [C].States='2') or (CHARINDEX(',", this.Session["username"], ",',','+[C].ZdUsername+',') > 0 and [C].States='3') or ([C].States='1'))" });
                str3 = str + SqlCreate;
                str4 = str2 + SqlCreate;
                str5 = "" + str3 + " " + Sqlsort + "";
            ...
            else
            {
                ...
                str2 = string.Concat(new object[] { "select count(A.id) from [qp_oa_TupianMg] as [A] join [qp_oa_TupianLx] as [C] on [A].[TypeId] = [C].id  and   ((CHARINDEX(',", this.Session["BuMenId"], ",',','+[C].ZdBumenId+',') > 0 and [C].States='2') or (CHARINDEX(',", this.Session["username"], ",',','+[C].ZdUsername+',') > 0 and [C].States='3') or ([C].States='1')) and A.TypeId='", base.Request.QueryString["id"], "'" });
                str3 = str + SqlCreate;
                str4 = str2 + SqlCreate;
                str5 = "" + str3 + " " + Sqlsort + "";
                this.GridView1.DataSource = this.List.GetGrid_Pages_not(str5);
