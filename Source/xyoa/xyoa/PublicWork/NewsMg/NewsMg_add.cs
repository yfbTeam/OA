namespace xyoa.PublicWork.NewsMg
{
    ...
    public class NewsMg_add : Page
    {
        ...
        {
            this.pulicss.QuanXianBack("jjjj1ba", this.Session["perstr"].ToString());
        ...
        {
            string sql = "insert into qp_oa_NewsMg  (YdUsername,YdRealname,ScUsername,Number,titles,Content,Settime,Username,Realname) values ('','','','" + this.pulicss.GetFormatStr(this.Number.Text) + "','" + this.pulicss.GetFormatStr(this.titles.Text) + "','" + this.pulicss.GetFormatStrbjq(this.Content.Value) + "','" + DateTime.Now.ToString() + "','" + this.Session["Username"].ToString() + "','" + this.Session["Realname"].ToString() + "')";
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("新增新闻", "新闻管理");
