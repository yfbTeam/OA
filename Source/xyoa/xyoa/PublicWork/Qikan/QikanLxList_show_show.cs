namespace xyoa.PublicWork.Qikan
{
    ...
    public class QikanLxList_show_show : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                list.Close();
                this.DataBindToGridview();
                string str2 = string.Concat(new object[] { "select * from qp_oa_QikanMg where id='", base.Request.QueryString["id"], "' and  CHARINDEX(',", this.Session["username"], ",',','+YdUsername+',')   >0" });
                SqlDataReader reader2 = this.List.GetList(str2);
                if (!reader2.Read())
                {
                    string str3 = string.Concat(new object[] { "Update qp_oa_QikanMg   Set YdUsername=YdUsername+'", this.Session["username"], ",',YdRealname=YdRealname+'", this.Session["realname"], ",'  where id='", int.Parse(base.Request.QueryString["id"]), "' " });
