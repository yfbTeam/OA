namespace xyoa.HumanResources.Hetong.HetongCZ
{
    ...
    public class HetongXQ_show_lsz : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                string sql = "select * from qp_hr_HetongXQ where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    this.Qixian.Text = list["Qixian"].ToString();
