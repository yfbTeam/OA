namespace xyoa.HumanResources.Gongzi.GongziMG
{
    ...
    public class GongziMG_showYsp : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                string sql = string.Concat(new object[] { "select * from qp_hr_GongziSB where id='", this.pulicss.GetFormatStr(base.Request.QueryString["id"]), "' and (CHARINDEX(',", this.Session["username"], ",',','+YspUsername+',')   >   0 )" });
                ...
                {
                    base.Response.Write("<script language=javascript>alert('无相关数据！');window.location.href='GongziMG.aspx?Zhuangtai=2'</script>");
                }
                list.Close();
