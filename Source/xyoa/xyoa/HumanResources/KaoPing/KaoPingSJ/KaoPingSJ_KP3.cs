namespace xyoa.HumanResources.KaoPing.KaoPingSJ
{
    ...
    public class KaoPingSJ_KP3 : Page
    {
        ...
        {
            string str;
            if (int.Parse(this.ViewState["Jishu"].ToString()) > 3)
            {
                str = string.Concat(new object[] { "Update qp_hr_KaoPingMgMx  Set Neirong3='", this.pulicss.GetFormatStrbjq(this.ContractContent.Text), "',Fenshu3='", this.pulicss.GetFormatStr(this.Fenshu.Text), "',KpZhuangtai3='2',KpZhuangtai4='1',Jieduan='4' where id='", int.Parse(base.Request.QueryString["id"]), "' and ((KpZhuangtai3='1' and User3='", this.Session["Username"], "'))" });
            ...
            {
                str = string.Concat(new object[] { "Update qp_hr_KaoPingMgMx  Set Neirong3='", this.pulicss.GetFormatStrbjq(this.ContractContent.Text), "',Fenshu3='", this.pulicss.GetFormatStr(this.Fenshu.Text), "',KpZhuangtai3='2',Jieduan='6' where id='", int.Parse(base.Request.QueryString["id"]), "' and ((KpZhuangtai3='1' and User3='", this.Session["Username"], "'))" });
                this.List.ExeSql(str);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                string sql = string.Concat(new object[] { "select * from qp_hr_KaoPingMgMx as [A] where id='", this.pulicss.GetFormatStr(base.Request.QueryString["id"]), "' and ((A.KpZhuangtai3='1' and A.User3='", this.Session["Username"], "'))" });
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    this.ViewState["User3"] = list["User4"].ToString();
                    this.ViewState["Name3"] = list["Name4"].ToString();
                    ...
                    this.Quanzhong.Text = list["Quanzhong3"].ToString();
                    this.ViewState["Jieduan"] = list["Jieduan"].ToString();
                    this.ViewState["Jishu"] = list["Jishu"].ToString();
                    this.TextBox1.Text = list["Neirong3"].ToString().Replace("disabled=\"disabled\"", "").Replace("disabled", "");
                    ...
                    if (reader2.Read())
                    {
                        ...
                        if (list["KpZhuangtai3"].ToString() == "2")
                        {
                            ...
                            this.Button1.Text = "三级考评已完成，不能再进行考评";
                        }
