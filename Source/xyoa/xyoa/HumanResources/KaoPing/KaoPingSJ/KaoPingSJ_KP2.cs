namespace xyoa.HumanResources.KaoPing.KaoPingSJ
{
    ...
    public class KaoPingSJ_KP2 : Page
    {
        ...
        {
            string str;
            if (int.Parse(this.ViewState["Jishu"].ToString()) > 2)
            {
                str = string.Concat(new object[] { "Update qp_hr_KaoPingMgMx  Set Neirong2='", this.pulicss.GetFormatStrbjq(this.ContractContent.Text), "',Fenshu2='", this.pulicss.GetFormatStr(this.Fenshu.Text), "',KpZhuangtai2='2',KpZhuangtai3='1',Jieduan='3' where id='", int.Parse(base.Request.QueryString["id"]), "' and ((KpZhuangtai2='1' and User2='", this.Session["Username"], "'))" });
            ...
            {
                str = string.Concat(new object[] { "Update qp_hr_KaoPingMgMx  Set Neirong2='", this.pulicss.GetFormatStrbjq(this.ContractContent.Text), "',Fenshu2='", this.pulicss.GetFormatStr(this.Fenshu.Text), "',KpZhuangtai2='2',Jieduan='6' where id='", int.Parse(base.Request.QueryString["id"]), "' and ((KpZhuangtai2='1' and User2='", this.Session["Username"], "'))" });
                this.List.ExeSql(str);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                string sql = string.Concat(new object[] { "select * from qp_hr_KaoPingMgMx as [A] where id='", this.pulicss.GetFormatStr(base.Request.QueryString["id"]), "' and ((A.KpZhuangtai2='1' and A.User2='", this.Session["Username"], "'))" });
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    this.ViewState["User3"] = list["User3"].ToString();
                    this.ViewState["Name3"] = list["Name3"].ToString();
                    ...
                    this.Quanzhong.Text = list["Quanzhong2"].ToString();
                    this.ViewState["Jieduan"] = list["Jieduan"].ToString();
                    this.ViewState["Jishu"] = list["Jishu"].ToString();
                    this.TextBox1.Text = list["Neirong2"].ToString().Replace("disabled=\"disabled\"", "").Replace("disabled", "");
                    ...
                    if (reader2.Read())
                    {
                        ...
                        if (list["KpZhuangtai2"].ToString() == "2")
                        {
                            ...
                            this.Button1.Text = "二级考评已完成，不能再进行考评";
                        }
