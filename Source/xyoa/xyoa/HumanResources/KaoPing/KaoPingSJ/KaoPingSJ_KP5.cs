namespace xyoa.HumanResources.KaoPing.KaoPingSJ
{
    ...
    public class KaoPingSJ_KP5 : Page
    {
        ...
        {
            string sql = string.Concat(new object[] { "Update qp_hr_KaoPingMgMx  Set Neirong5='", this.pulicss.GetFormatStrbjq(this.ContractContent.Text), "',Fenshu5='", this.pulicss.GetFormatStr(this.Fenshu.Text), "',KpZhuangtai5='2',Jieduan='6' where id='", int.Parse(base.Request.QueryString["id"]), "' and ((KpZhuangtai5='1' and User5='", this.Session["Username"], "'))" });
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                string sql = string.Concat(new object[] { "select * from qp_hr_KaoPingMgMx as [A] where id='", this.pulicss.GetFormatStr(base.Request.QueryString["id"]), "' and ((A.KpZhuangtai5='1' and A.User5='", this.Session["Username"], "'))" });
                ...
                if (list.Read())
                {
                    ...
                    this.Quanzhong.Text = list["Quanzhong5"].ToString();
                    this.ViewState["Jieduan"] = list["Jieduan"].ToString();
                    this.ViewState["Jishu"] = list["Jishu"].ToString();
                    this.TextBox1.Text = list["Neirong5"].ToString().Replace("disabled=\"disabled\"", "").Replace("disabled", "");
                    ...
                    if (reader2.Read())
                    {
                        ...
                        if (list["KpZhuangtai5"].ToString() == "2")
                        {
                            ...
                            this.Button1.Text = "五级考评已完成，不能再进行考评";
                        }
