namespace xyoa.HumanResources.PeiXun.MyPage
{
    ...
    public class PeiXun : Page
    {
        ...
        {
            this.pulicss.QuanXianVis(this.GridView1, "eeee2a", this.Session["perstr"].ToString());
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.*,D.[Mingcheng] as Mingcheng from [qp_hr_Peixun] as [A]  inner join [qp_hr_PeixunLb] as [D] on [A].[Leibie] = [D].[Id] where (CHARINDEX('," + this.Session["username"] + ",',','+UsernameCj+',') > 0)";
            string str2 = "select count(id) from qp_hr_Peixun as [A] where (CHARINDEX('," + this.Session["username"] + ",',','+UsernameCj+',') > 0)";
