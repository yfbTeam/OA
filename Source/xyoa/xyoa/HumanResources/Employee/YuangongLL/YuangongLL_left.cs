namespace xyoa.HumanResources.Employee.YuangongLL
{
    ...
    public class YuangongLL_left : Page
    {
        ...
        private void BindTree(TreeNodeCollection Nds, int IDStr)
        {
            ...
            while (list.Read())
            {
                ...
                if (IDStr == 0)
                {
                    str2 = "select count(A.id) from  [qp_hr_Yuangong] as [A] where A.Bumen='" + list["id"] + "' and Zaizhi='1'";
                    ...
                    str4 = "select id,Xingming  from [qp_hr_Yuangong] where Bumen='" + list["ID"].ToString() + "' and Zaizhi='1' order by A.id asc";
