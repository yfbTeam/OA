        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                try
                {
                    ...
                    string sql = string.Concat(new object[] { "select count(A.id) from [qp_hr_HetongMg] as [A]  inner join [qp_hr_HetongMB] as [D] on [A].[LeibieID] = [D].[Id] inner join [qp_oa_username] as [C] on [A].[QyUsername] = [C].[Username] inner join [qp_oa_Bumen] as [B] on [B].[id] = [C].[BuMenId] where ((Endtime between '", this.ViewState["Starttime"], "'and  '", this.ViewState["Endtime"], "') or datediff(dd,Endtime,'", this.ViewState["Starttime"], "')=0 or datediff(dd,Endtime,'", this.ViewState["Endtime"], "')=0) and A.Qixian=1" });
                    ...
                    else
                    {
                        ...
                        while (list.Read())
                        {
                            ...
                            object obj2;
                            string str4 = string.Concat(new object[] { "select count(A.id) from [qp_hr_HetongMg] as [A]  inner join [qp_hr_HetongMB] as [D] on [A].[LeibieID] = [D].[Id] inner join [qp_oa_username] as [C] on [A].[QyUsername] = [C].[Username] inner join [qp_oa_Bumen] as [B] on [B].[id] = [C].[BuMenId] where Year(Endtime)='", list["ProYear"], "' and  Month(Endtime)='", list["ProMonth"], "' and A.Qixian=1 " });
