namespace xyoa.HumanResources.Fenxi.RenShi
{
    ...
    public class Fenxi19List : Page
    {
        ...
        {
            base.Response.Redirect("Fenxi19.aspx");
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                try
                {
                    ...
                    string sql = string.Concat(new object[] { "select count(A.id) from [qp_hr_YuangongLZ] as [A]  inner join [qp_hr_YuangongLx] as [F] on [A].[Leixing] = [F].[id] where ((Riqi between '", this.ViewState["Starttime"], "'and  '", this.ViewState["Endtime"], "') or datediff(dd,Riqi,'", this.ViewState["Starttime"], "')=0 or datediff(dd,Riqi,'", this.ViewState["Endtime"], "')=0)" });
                    ...
                    else
                    {
                        ...
                        while (list.Read())
                        {
                            ...
                            object obj2;
                            string str4 = string.Concat(new object[] { "select count(A.id) from [qp_hr_YuangongLZ] as [A]  inner join [qp_hr_YuangongLx] as [F] on [A].[Leixing] = [F].[id] where Year(Riqi)='", list["ProYear"], "' and  Month(Riqi)='", list["ProMonth"], "' " });
