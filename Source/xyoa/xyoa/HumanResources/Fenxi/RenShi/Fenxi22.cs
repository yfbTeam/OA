    ...
    public class Fenxi22 : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                try
                {
                    string sql = "select count(A.id) from  [qp_hr_YuangongDD] as [A]  where 1=1";
                    ...
                    else
                    {
                        ...
                        object obj2;
                        string str3 = "select id,name from qp_hr_YuangongLx where type='8' order by id asc";
                        int num = 1;
                        SqlDataReader list = this.List.GetList(str3);
                        while (list.Read())
                        {
                            string str4 = "select count(A.id) from [qp_hr_YuangongDD] as [A]  inner join [qp_oa_Bumen] as [B] on [A].[BumenQ] = [B].[Id] inner join [qp_oa_Bumen] as [C] on [A].[BumenE] = [C].[Id] inner join [qp_oa_Zhiwei] as [D] on [A].[ZhiweiQ] = [D].[id]  inner join [qp_oa_Zhiwei] as [E] on [A].[ZhiweiE] = [E].[id]   inner join [qp_hr_YuangongLx] as [F] on [A].[Leixing] = [F].[id] where A.Leixing='" + list["id"] + "'";
