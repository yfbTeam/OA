    ...
    public class Fenxi23 : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                try
                {
                    string sql = "select count(A.id) from  [qp_hr_YuangongLZ] as [A]  where 1=1";
                    ...
                    else
                    {
                        ...
                        object obj2;
                        string str3 = "select id,name from qp_hr_YuangongLx where type='9' order by id asc";
                        int num = 1;
                        SqlDataReader list = this.List.GetList(str3);
                        while (list.Read())
                        {
                            string str4 = "select count(A.id) from [qp_hr_YuangongLZ] as [A]  inner join [qp_hr_YuangongLx] as [F] on [A].[Leixing] = [F].[id] where A.Leixing='" + list["id"] + "'";
