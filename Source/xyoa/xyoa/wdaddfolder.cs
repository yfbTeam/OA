        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                else
                {
                    ...
                    string sql = "select * from qp_oa_WenDangMg  where NewName='" + base.Request.QueryString["NewName"] + "'";
                    SqlDataReader list = this.List.GetList(sql);
                    if (list.Read())
                    {
                        this.OldName.Text = list["OldName"].ToString();
                        string str2 = string.Concat(new object[] { "select * from qp_oa_WenDangMg where NewName='", base.Server.UrlDecode(base.Request.QueryString["number"]), "' and  CHARINDEX(',", this.Session["username"], ",',','+YdUsername+',')   >0" });
                        SqlDataReader reader2 = this.List.GetList(str2);
                        if (!reader2.Read())
                        {
                            string str3 = string.Concat(new object[] { "Update qp_oa_WenDangMg   Set YdUsername=YdUsername+'", this.Session["username"], ",',YdRealname=YdRealname+'", this.Session["realname"], ",'  where NewName='", base.Server.UrlDecode(base.Request.QueryString["number"]), "' " });
