namespace xyoa.InfoManage.QingShi
{
    ...
    public class MyQingShi_update : Page
    {
        ...
        {
            string sql = string.Concat(new object[] { "Update qp_oa_QingShi  Set  Titles='", this.pulicss.GetFormatStr(this.Titles.Text), "',Content='", this.pulicss.GetFormatStrbjq(this.Content.Value), "',JsUsername='", this.pulicss.GetFormatStr(this.JsUsername.Text), "',JsRealname='", this.pulicss.GetFormatStr(this.JsRealname.Text), "',TxTime='", DateTime.Now.ToString(), "' where id='", int.Parse(base.Request.QueryString["id"]), "'  and username='", this.Session["username"], "'  " });
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("修改请示报告", "请示报告");
            if (this.C1.Checked)
            {
                this.pulicss.InsertMessage(string.Concat(new object[] { "您有新的请示报告需要批阅：", this.Titles.Text, "，请示人：", this.Session["realname"], "" }), this.JsUsername.Text, this.JsRealname.Text, "/InfoManage/QingShi/QingShiList.aspx");
            }
            ...
            else
            {
                if (!this.Page.IsPostBack)
                {
                    ...
                    if (list.Read())
                    {
                        ...
                        if (list["State"].ToString() == "已批阅")
                        {
                            this.Button1.Enabled = false;
                            this.Button1.Text = "已批阅，不能修改";
                        }
                    }
                    list.Close();
                    this.BindAttribute();
