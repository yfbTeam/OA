namespace xyoa.InfoManage.zhiao
{
    ...
    public class MyPage_ph : Page
    {
        ...
        {
            base.Response.Redirect("MyPage_ph.aspx");
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.jifen,A.Username, A.Realname,B.[Name] as BuMenName from [qp_oa_username] as [A] inner join [qp_oa_Bumen] as [B] on [A].[BuMenId] = [B].[Id] where 1=1  and  ifdel='0'";
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                Label label = (Label) e.Row.FindControl("jifen");
                Label label2 = (Label) e.Row.FindControl("dengji");
                label2.Text = null;
                string sql = "select dengji from qp_oa_Zst_dengji where  " + label.Text + ">=Fenshu1 and  " + label.Text + "<=Fenshu2";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    label2.Text = "<img src='/InfoManage/zhiao/img/d_" + list["dengji"] + ".gif' />";
                }
                else
                {
                    label2.Text = "无等级";
