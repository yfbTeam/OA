namespace xyoa.InfoManage.zhiao
{
    ...
    public class wenti_wjj : Page
    {
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str2 = string.Concat(new object[] { "select count(A.id)  from [qp_oa_Zst_wenti] as [A] inner join [qp_oa_Zst_leibie_wt] as [B] on [A].[Leibie] = [B].[Id]  where  jiejue='2' and ((CHARINDEX(',", this.Session["BuMenId"], ",',','+B.ZdBumenId+',') > 0 and B.States='2') or (CHARINDEX(',", this.Session["username"], ",',','+B.ZdUsername+',') > 0 and B.States='3') or (B.States='1'))" });
