namespace xyoa.InfoManage.zhiao
{
    ...
    public class MyPage_zl : Page
    {
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.cishu,A.Leibie, A.Name, A.NewName,A.Filetype, A.Settimes,A.Username,A.Realname, B.[Name] as LeibieName from [qp_oa_Zst_ziliao] as [A] inner join [qp_oa_Zst_leibie_zl] as [B] on [A].[Leibie] = [B].[Id] where Username='" + this.Session["Username"] + "'";
            string str2 = "select count(A.id)  from [qp_oa_Zst_ziliao] as [A] inner join [qp_oa_Zst_leibie_zl] as [B] on [A].[Leibie] = [B].[Id] where Username='" + this.Session["Username"] + "'";
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "syguihuan")
            {
                ...
                string sql = "delete from qp_oa_Zst_ziliao where id='" + num + "'";
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                label3.Text = "<a href='MyPage_zl_update.aspx?id=" + label.Text + "'>修改</a>";
                ...
                label5.Text = null;
                string sql = "select id,Name,ParentNodesID from qp_oa_Zst_leibie_zl where  id='" + label4.Text + "'";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    label5.Text = string.Concat(new object[] { "<a href=\"ziliao_xiaolei.aspx?ParentNodesID=", list["ParentNodesID"], "&id=", list["id"], "\" title=\"", this.pulicss.TypeCode("qp_oa_Zst_leibie_zl", list["ParentNodesID"].ToString()), " >> ", list["Name"], "\"  target=_parent>", list["Name"], "</a>" });
                }
                list.Close();
            }
        }
        ...
        protected void LinkButton1_Click(object sender, EventArgs e)
        {
            ...
            string sql = "  Delete from qp_oa_Zst_ziliao  where ID in (" + str + ")";
            this.List.ExeSql(sql);
            this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
        }
        ...
        {
            base.Response.Redirect("MyPage_zl_add.aspx");
        }
