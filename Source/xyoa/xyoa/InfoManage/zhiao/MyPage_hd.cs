namespace xyoa.InfoManage.zhiao
{
    ...
    public class MyPage_hd : Page
    {
        ...
        {
            base.Response.Redirect("MyPage_hd.aspx");
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.tuijian,A.Dianji,A.Huida,A.jiejue,A.Leibie, A.Zhuti, A.Settimes,A.Username,A.Realname, B.[Name] as LeibieName from [qp_oa_Zst_wenti] as [A] inner join [qp_oa_Zst_leibie_wt] as [B] on [A].[Leibie] = [B].[Id] where CHARINDEX('," + this.Session["username"] + ",',','+UsernameHd+',')   >0";
            string str2 = "select count(A.id)  from [qp_oa_Zst_wenti] as [A] inner join [qp_oa_Zst_leibie_wt] as [B] on [A].[Leibie] = [B].[Id] where CHARINDEX('," + this.Session["username"] + ",',','+UsernameHd+',')   >0";
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (list.Read())
                {
                    label2.Text = string.Concat(new object[] { "<a href=\"wenti_xiaolei.aspx?ParentNodesID=", list["ParentNodesID"], "&id=", list["id"], "\" title=\"", this.pulicss.TypeCode("qp_oa_Zst_leibie_wt", list["ParentNodesID"].ToString()), " >> ", list["Name"], "\"  target=_parent>", list["Name"], "</a>" });
                }
                list.Close();
            }
        }
