namespace xyoa.InfoManage.zhiao
{
    ...
    public class MyPage_wt_update : Page
    {
        ...
        {
            string sql = string.Concat(new object[] { "Update qp_oa_Zst_wenti  Set  Neirong='", this.pulicss.GetFormatStr(this.Content.Text), "',Zhuti='", this.pulicss.GetFormatStr(this.Zhuti.Text), "',Leibie='", this.pulicss.GetFormatStr(this.Leibie.Text), "' where id='", int.Parse(base.Request.QueryString["id"]), "'  and Username='", this.Session["Username"], "'  " });
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("修改知识堂问题", "知识堂问题");
            ...
            else
            {
                if (!this.Page.IsPostBack)
                {
                    ...
                    if (list.Read())
                    {
                        ...
                        this.LeibieName.Text = this.pulicss.TypeCode("qp_oa_Zst_leibie_wt", list["Leibie"].ToString());
                        this.Leibie.Text = list["Leibie"].ToString();
                        this.Zhuti.Text = list["Zhuti"].ToString();
                        this.Content.Text = list["Neirong"].ToString();
