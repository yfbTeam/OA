namespace xyoa.InfoManage.zhiao
{
    ...
    public class leibie_wt_update : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            string sql = string.Concat(new object[] { 
                "Update qp_oa_Zst_leibie_wt  Set  Name='", this.pulicss.GetFormatStr(this.Name.Text), "',ParentNodesID='", this.ParentNodesID.SelectedValue, "',States='", this.States.SelectedValue, "',ZdBumenId='", this.ZdBumenId.Text, "',ZdBumen='", this.ZdBumen.Text, "',ZdUsername='", this.ZdUsername.Text, "',ZdRealname='", this.ZdRealname.Text, "',Paixun='", this.Paixun.Text, 
                "'  where id='", int.Parse(base.Request.QueryString["id"]), "'  "
             });
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("修改资料分类设置", "资料分类设置");
            base.Response.Write("<script language=javascript>alert('修改成功！'); window.parent.location = 'leibie_wt.aspx'</script>");
        }

        protected void Button3_Click(object sender, EventArgs e)
        {
            string sql = "  Delete from qp_oa_Zst_leibie_wt  where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'";
            this.List.ExeSql(sql);
            string str2 = "  Delete from qp_oa_Zst_leibie_wt  where ParentNodesID='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'";
            this.List.ExeSql(str2);
            this.pulicss.InsertLog("删除资料分类设置", "资料分类设置");
            base.Response.Write("<script language=javascript>alert('删除成功！'); window.parent.location = 'leibie_wt.aspx'</script>");
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                string sql = "select * from qp_oa_Zst_leibie_wt  where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'  ";
