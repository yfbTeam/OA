namespace xyoa.InfoManage.sms
{
    ...
    public class MySms_Shenpi_show : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                string str2;
                str = string.Concat(new object[] { "Update qp_oa_shouji  Set YjbUser=YjbUser+'", this.Session["username"], ",',DqBlUsername=replace(DqBlUsername,'", this.Session["username"].ToString(), ",','') where id='", int.Parse(base.Request.QueryString["id"]), "'" });
                ...
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='MySms_Shenpi.aspx'</script>");
            }
            else if (this.ZbRealname.Text != "")
            {
                ...
                while (list.Read())
                {
                    this.pulicss.InsertMessage("您有手机短信发送需要审批", list["username"].ToString(), list["realname"].ToString(), "/InfoManage/sms/MySms_Shenpi.aspx");
                }
                list.Close();
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='MySms_Shenpi.aspx'</script>");
            }
        ...
        protected void Button4_Click(object sender, EventArgs e)
        {
            ...
            {
                string str2;
                str = string.Concat(new object[] { "Update qp_oa_shouji  Set DqBlUsername=replace(DqBlUsername,'", this.Session["username"].ToString(), ",','') where id='", int.Parse(base.Request.QueryString["id"]), "'" });
                this.List.ExeSql(str);
                if (this.Yijian.Text.Trim() != "")
                {
                    str2 = string.Concat(new object[] { 
            ...
            else
            {
                ...
                this.pulicss.SpUpdateLog(this.Number.Text, this.Yijian.Text, "3", this.GlNum.Text, this.CaoZuo.SelectedValue, DateTime.Now.ToString());
                str = string.Concat(new object[] { "Update qp_oa_shouji  Set  DqNodeId='',DqBlUsername='已结束',DqBlXinming='已结束',LcZhuangtai='3',YjbUser=YjbUser+'", this.Session["username"], ",',GlNum='", str3, "',YjbNodeId=YjbNodeId+'", this.YjbNodeId.Value, ",',DqNodeName='已结束',SNodeId='", this.YjbNodeId.Value, "' where id='", int.Parse(base.Request.QueryString["id"]), "'  " });
                this.List.ExeSql(str);
                this.pulicss.InsertMessage("您手机短信发送，已审批完成", this.Username.Text, this.Realname.Text, "/InfoManage/sms/MySms_Shenpi.aspx");
                if (this.acceptusername.Text.Trim() != "")
                {
                    ...
                    {
                        str6 = "select MoveTel from qp_oa_Username";
                        reader = this.List.GetList(str6);
                        while (reader.Read())
                        {
                            this.pulicss.InsertSmsSjUser(reader["MoveTel"].ToString(), this.Content.Text, this.Starttime.Text, this.Username.Text, this.Realname.Text);
                        }
                        reader.Close();
                    }
                    else
                    {
                        str6 = "select MoveTel from qp_oa_Username where  username in (" + str4 + ") ";
                        reader = this.List.GetList(str6);
                        while (reader.Read())
                        {
                            this.pulicss.InsertSmsSjUser(reader["MoveTel"].ToString(), this.Content.Text, this.Starttime.Text, this.Username.Text, this.Realname.Text);
                ...
                {
                    string str7 = null;
                    str7 = "" + this.wbUsername.Text + "";
                    ArrayList list2 = new ArrayList();
                    string[] strArray2 = str7.Split(new char[] { ',' });
                    for (int j = 0; j < strArray2.Length; j++)
                    {
                        this.pulicss.InsertSmsSjUser(strArray2[j], this.Content.Text, this.Starttime.Text, this.Username.Text, this.Realname.Text);
                    }
                }
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='MySms_Shenpi.aspx'</script>");
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                string sql = string.Concat(new object[] { "select * from qp_oa_shouji   where id='", this.pulicss.GetFormatStr(base.Request.QueryString["id"]), "' and CHARINDEX(',", this.Session["username"], ",',','+DqBlUsername+',')   >0" });
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
        ...
        {
            int num = 0;
            string sql = "select DqBlUsername from qp_oa_shouji   where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'";
            SqlDataReader list = this.List.GetList(sql);
            if (list.Read())
            {
