namespace xyoa.InfoManage.bbs
{
    ...
    public class InsideBBSListAdd : Page
    {
        ...
        {
            string sql = string.Concat(new object[] { "insert into qp_oa_InsideBBS  (Number,Titles,Content,BigId,ParentNodesID,PointNum,Username,Realname,Settime) values ('", this.pulicss.GetFormatStr(this.Number.Text), "','", this.pulicss.GetFormatStr(this.titles.Text), "','", this.pulicss.GetFormatStrbjq(this.Content.Value), "','", base.Request.QueryString["id"], "','0','0','", this.Session["username"], "','", this.Session["realname"], "','", DateTime.Now.ToString(), "')" });
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("发帖" + this.titles.Text + "", "论坛BBS");
            base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='InsideBBSList.aspx?id=" + base.Request.QueryString["id"] + "'</script>");
        }
