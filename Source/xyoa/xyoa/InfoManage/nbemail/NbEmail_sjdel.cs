namespace xyoa.InfoManage.nbemail
{
    ...
    public class NbEmail_sjdel : Page
    {
        ...
        {
            base.Response.Redirect("NbEmail_sjdel.aspx");
        }

        protected void Button3_Click(object sender, EventArgs e)
        {
            string str = this.CheckCbxDel();
            string sql = "Update qp_oa_NbEmail_sj  Set IfDel='0' where ID in (" + str + ")";
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("还原内部邮件", "已删除");
            this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select  id,Titles,IfRead,FsUsername,FsRealname,FsTime,Number  from qp_oa_NbEmail_sj  where IfDel='1' and  JsUsername='" + this.Session["username"] + "'";
            string str2 = "select count(id) from qp_oa_NbEmail_sj where  IfDel='1' and  JsUsername='" + this.Session["username"] + "'";
        ...
        {
            string str = this.CheckCbxDel();
            string sql = "  Delete from qp_oa_NbEmail_sj  where ID in (" + str + ")";
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("删除内部邮件", "已删除");
            this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
        }
