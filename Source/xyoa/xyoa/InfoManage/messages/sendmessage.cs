namespace xyoa.InfoManage.messages
{
    ...
    public class sendmessage : Page
    {
        ...
        {
            this.pulicss.QuanXianVis(this.IMG1, ",17,", this.pulicss.GetSms());
            this.pulicss.QuanXianVis(this.CheckBox2, ",17,", this.pulicss.GetSms());
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                string sql = "select username,realname from qp_oa_username where username='" + this.pulicss.GetFormatStr(base.Request.QueryString["user"]) + "'";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    this.acceptusername.Text = "" + list["username"].ToString() + ",";
                    this.acceptrealname.Text = "" + list["realname"].ToString() + ",";
                }
                list.Close();
                this.BindAttribute();
            }
