namespace xyoa.OpenWindows
{
    ...
    public class open_user_list_fzr : Page
    {
        ...
        public void DataBindToGridview(string Sqlsort, string Sqlstr)
        {
            ...
            if (base.Request.QueryString["requeststr"] != null)
            {
                ...
                {
                    str3 = string.Concat(new object[] { "select username, Realname  from qp_oa_username  where 1=1 and  ifdel='0'  and username  in (", str, ") and username not in (", this.ViewState["strlist"], ")" });
                }
                else
                {
                    str3 = "select username, Realname  from qp_oa_username  where 1=1 and  ifdel='0'  and username not in (" + this.ViewState["strlist"] + ")";
                }
                sQL = str3 + Sqlstr + Sqlsort;
            ...
            else
            {
                if (base.Request.QueryString["user"].ToString() != "")
                {
                    str3 = "select username, Realname  from qp_oa_username  where 1=1 and  ifdel='0'  and username  in (" + str + ")";
                }
                else
                {
                    str3 = "select username, Realname  from qp_oa_username  where 1=1 and  ifdel='0'  ";
                }
                sQL = str3 + Sqlstr + Sqlsort;
                this.list.Bind_DropDownList_ListBox(this.SourceListBox, sQL, "Username", "Realname");
            }
