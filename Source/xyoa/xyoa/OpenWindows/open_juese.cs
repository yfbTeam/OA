namespace xyoa.OpenWindows
{
    ...
    public class open_juese : Page
    {
        ...
        {
            base.Response.Redirect("open_juese.aspx?key=1&name=" + this.name.Text + "&user=" + base.Request.QueryString["user"].ToString() + "&requeststr=" + base.Server.UrlDecode(base.Request.QueryString["requeststr"].ToString()) + "");
        }
        ...
        public void DataBindToGridview()
        {
            ...
            {
                str = "select id,Name from  qp_oa_Juese where id in (" + base.Request.QueryString["user"].ToString() + "0) and 1=1";
            }
            else
            {
                str = "select id,Name from  qp_oa_Juese where 1=1";
            }
            ...
            else
            {
                ...
                if (base.Request.QueryString["user"].ToString() != "")
                {
                    str4 = "select id,Name from qp_oa_Juese where id in (" + base.Request.QueryString["user"].ToString() + "0) and 1=1  order by id asc";
                }
                else
                {
                    str4 = "select id,Name from qp_oa_Juese where  1=1  order by id asc";
                }
                this.GridView1.DataSource = this.List.GetGrid_Pages_not(str4);
                this.GridView1.DataBind();
            }
