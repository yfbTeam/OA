namespace xyoa.OpenWindows
{
    ...
    public class open_user_list_Zw : Page
    {
        ...
        public void DataBindToGridview(string Sqlsort, string Sqlstr)
        {
            ...
            if (base.Request.QueryString["requeststr"] != null)
            {
                str2 = string.Concat(new object[] { "select username, Realname  from qp_oa_username  where Zhiweiid = '", base.Request.QueryString["Zhiweiid"], "' and  ifdel='0'  and username not in (", this.ViewState["strlist"], ")" }) + Sqlstr + Sqlsort;
                this.list.Bind_DropDownList_ListBox(this.SourceListBox, str2, "Username", "Realname");
            }
            else
            {
                str2 = ("select username, Realname  from qp_oa_username  where Zhiweiid = '" + base.Request.QueryString["Zhiweiid"] + "' and  ifdel='0'  ") + Sqlstr + Sqlsort;
                this.list.Bind_DropDownList_ListBox(this.SourceListBox, str2, "Username", "Realname");
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (!this.Page.IsPostBack)
                {
                    ...
                    this.list.Bind_DropDownList_kong(this.JueseId, sQL, "id", "name");
                    if (base.Request.QueryString["requeststr"] != null)
                    {
                        string str3 = null;
