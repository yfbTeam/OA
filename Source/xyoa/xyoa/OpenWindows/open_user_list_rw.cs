namespace xyoa.OpenWindows
{
    ...
    public class open_user_list_rw : Page
    {
        ...
        protected void Button10_Click(object sender, EventArgs e)
        {
            ...
            {
                string sQL = null;
                sQL = string.Concat(new object[] { "select * from qp_oa_Username  where id in (", this.ViewState["strlist"], ") and  ifdel='0' and CHARINDEX(','+Username+',',','+(select RyUsername from qp_oa_Renwusz where ZgUsername='", this.Session["username"], "')+',') > 0  order by paixu asc" });
        ...
        public string CreateMidSql()
        {
            ...
            reader.Close();
            string str5 = " and username in (" + str2 + ")";
            return (str + str5);
        }
        ...
        public void DataBindToGridview(string Sqlsort, string Sqlstr)
        {
            ...
            if (base.Request.QueryString["requeststr"] != null)
            {
                str2 = string.Concat(new object[] { "select id, Realname  from qp_oa_username  where 1=1 and id not in (", this.ViewState["strlist"], ") and  ifdel='0' and CHARINDEX(','+Username+',',','+(select RyUsername from qp_oa_Renwusz where ZgUsername='", this.Session["username"], "')+',') > 0" }) + Sqlstr + Sqlsort;
                this.list.Bind_DropDownList_ListBox(this.SourceListBox, str2, "id", "Realname");
            }
            else
            {
                str2 = ("select id, Realname  from qp_oa_username  where 1=1 and  ifdel='0' and CHARINDEX(','+Username+',',','+(select RyUsername from qp_oa_Renwusz where ZgUsername='" + this.Session["username"] + "')+',') > 0 ") + Sqlstr + Sqlsort;
                this.list.Bind_DropDownList_ListBox(this.SourceListBox, str2, "id", "Realname");
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                this.ViewState["strlist"] = "" + base.Request.QueryString["requeststr"].ToString() + "0";
