namespace xyoa.OpenWindows
{
    ...
    public class open_user_listht : Page
    {
        ...
        protected void Button10_Click(object sender, EventArgs e)
        {
            ...
            {
                string sQL = null;
                sQL = string.Concat(new object[] { "select * from qp_oa_Username  where username in (", this.ViewState["strlist"], ") and  ifdel='0'  and CHARINDEX(','+Username+',',','+(select RyUsername from qp_oa_HetongSz where ZgUsername='", this.Session["username"], "')+',') > 0  order by paixu asc " });
        ...
        public void DataBindToGridview(string Sqlsort, string Sqlstr)
        {
            ...
            if (base.Request.QueryString["requeststr"] != null)
            {
                str2 = string.Concat(new object[] { "select username, Realname  from qp_oa_username  where 1=1 and username not in (", this.ViewState["strlist"], ") and  ifdel='0' and CHARINDEX(','+Username+',',','+(select RyUsername from qp_oa_HetongSz where ZgUsername='", this.Session["username"], "')+',') > 0  " }) + Sqlstr + Sqlsort;
                this.list.Bind_DropDownList_ListBox(this.SourceListBox, str2, "Username", "Realname");
            }
            else
            {
                str2 = ("select username, Realname  from qp_oa_username  where 1=1 and  ifdel='0' and CHARINDEX(','+Username+',',','+(select RyUsername from qp_oa_HetongSz where ZgUsername='" + this.Session["username"] + "')+',') > 0   ") + Sqlstr + Sqlsort;
                this.list.Bind_DropDownList_ListBox(this.SourceListBox, str2, "Username", "Realname");
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (!this.Page.IsPostBack)
                {
                    ...
                    {
                        string str5 = null;
                        str5 = string.Concat(new object[] { "select * from qp_oa_Username  where username in (", this.ViewState["strlist"], ") and  ifdel='0' and CHARINDEX(','+Username+',',','+(select RyUsername from qp_oa_HetongSz where ZgUsername='", this.Session["username"], "')+',') > 0  order by paixu asc" });
                        this.TargetListBox.Items.Clear();
