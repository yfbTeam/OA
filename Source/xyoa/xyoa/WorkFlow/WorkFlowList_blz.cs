namespace xyoa.WorkFlow
{
    ...
    public class WorkFlowList_blz : Page
    {
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.UpTimeSet,A.Number,A.JinJiChengDu,A.Sequence,A.FormName,A.Name,A.FlowNumber,A.FqUsername,A.FqRealname,A.NodeName,A.UpNodeName,A.UpNodeId,A.FormId,C.StartTime as SjStartTime from [qp_oa_AddWorkFlow] as [A] inner join [qp_oa_AddWorkFlowPicRy] as [C] on [A].[Number] = [C].KeyFile  and [A].[UpNodeNum] = [C].xuhao  and [A].[GlNum] = [C].GlNum   where BLusername='" + this.Session["username"] + "' and States='办理中' and State='正在办理' and Ifdel='0'";
            string str2 = "select count(A.id) from [qp_oa_AddWorkFlow] as [A] inner join [qp_oa_AddWorkFlowPicRy] as [C] on [A].[Number] = [C].KeyFile  and [A].[UpNodeNum] = [C].xuhao  and [A].[GlNum] = [C].GlNum  where  BLusername='" + this.Session["username"] + "' and States='办理中' and State='正在办理' and Ifdel='0'";
