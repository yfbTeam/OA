namespace xyoa.WorkFlow
{
    ...
    public class WorkFlowList_ybj : Page
    {
        ...
        {
            base.Response.Redirect("WorkFlowList_blz.aspx");
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.UpTimeSet,A.Number,A.Sequence,A.FormName,A.Name,A.FlowNumber,A.FqUsername,A.FqRealname,A.NodeName,A.UpNodeName,A.UpNodeId,A.FormId,C.StartTime as SjStartTime from [qp_oa_AddWorkFlow] as [A] inner join [qp_oa_AddWorkFlowPicRy] as [C] on [A].[Number] = [C].KeyFile  and [A].[UpNodeNum] = [C].xuhao  and [A].[GlNum] = [C].GlNum  where BLusername='" + this.Session["username"] + "' and States='已办结' and State='正在办理' and Ifdel='0'";
            string str2 = "select count(A.id) from [qp_oa_AddWorkFlow] as [A] inner join [qp_oa_AddWorkFlowPicRy] as [C] on [A].[Number] = [C].KeyFile  and [A].[UpNodeNum] = [C].xuhao  and [A].[GlNum] = [C].GlNum  where  BLusername='" + this.Session["username"] + "' and States='已办结' and State='正在办理' and Ifdel='0'";
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "ShanChu")
            {
                ...
                string sql = "select Number,UpNodeNum,UpNodeName,FormName,Name,GlNum from qp_oa_AddWorkFlow where id='" + num + "'";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    string str2 = string.Concat(new object[] { "select id,IfZb from qp_oa_AddWorkFlowPicRy where KeyFile='", list["Number"].ToString(), "' and BLusername='", this.Session["username"], "' and XuHao='", list["UpNodeNum"], "' and GlNum='", list["GlNum"], "'" });
                    SqlDataReader reader2 = this.List.GetList(str2);
                    if (reader2.Read())
                    {
                        string str3 = "Update qp_oa_AddWorkFlowPicRy  Set States='办理中',EndTime=''  where id='" + reader2["id"].ToString() + "'";
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                Label label6 = (Label) e.Row.FindControl("SjStartTime");
                ...
                if (list.Read())
                {
                    if (list["Shuxing"].ToString() == "自由流程")
                    {
                        ...
                        label4.Text = "<a href='javascript:void(0)' onclick='var num = Math.random();var aw = screen.width-300;var ah = screen.height-100;loc_x=(screen.availWidth-aw)/2;loc_y=(screen.availHeight-ah)/2;window.open (\"WorkFlowList_show_zy.aspx?id=" + list["id"].ToString() + "&UpNodeId=" + list["UpNodeId"].ToString() + "&FlowNumber=" + list["FlowNumber"].ToString() + "&FormId=" + list["FormId"].ToString() + "&tmp=\" + num + \"\", \"_blank\", \"height=\" + ah + \", width=\" + aw + \",ttoolbar=no, menubar=no, scrollbars=no,resizable=yes,location=no, status=no,top=\" + loc_y + \",left=\" + loc_x + \"\")'>" + list["FormName"].ToString() + "</a>";
                    ...
                    else
                    {
                        ...
                        if (reader2.Read())
                        {
                            TimeSpan span = (TimeSpan) (DateTime.Parse(DateTime.Now.ToString()) - DateTime.Parse(label6.Text));
                        ...
                        label4.Text = "<a href='javascript:void(0)' onclick='var num = Math.random();var aw = screen.width-300;var ah = screen.height-100;loc_x=(screen.availWidth-aw)/2;loc_y=(screen.availHeight-ah)/2;window.open (\"WorkFlowList_show.aspx?id=" + list["id"].ToString() + "&UpNodeId=" + list["UpNodeId"].ToString() + "&FlowNumber=" + list["FlowNumber"].ToString() + "&FormId=" + list["FormId"].ToString() + "&tmp=\" + num + \"\", \"_blank\", \"height=\" + ah + \", width=\" + aw + \",ttoolbar=no, menubar=no, scrollbars=yes,resizable=yes,location=no, status=no,top=\" + loc_y + \",left=\" + loc_x + \"\")'>" + list["FormName"].ToString() + "</a>";
