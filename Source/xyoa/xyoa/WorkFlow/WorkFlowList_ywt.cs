namespace xyoa.WorkFlow
{
    ...
    public class WorkFlowList_ywt : Page
    {
        ...
        {
            base.Response.Redirect("WorkFlowList_ywt.aspx");
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.UpTimeSet,A.Number,A.Sequence,A.FormName,A.Name,A.FlowNumber,A.FqUsername,A.FqRealname,A.NodeName,A.UpNodeName,A.UpNodeId,A.FormId,C.StartTime as SjStartTime from [qp_oa_AddWorkFlow] as [A] inner join [qp_oa_AddWorkFlowPicRy] as [C] on [A].[Number] = [C].KeyFile  and [A].[UpNodeNum] = [C].xuhao  and [A].[GlNum] = [C].GlNum  where BLusername='" + this.Session["username"] + "' and States='已委托' and State='正在办理' and Ifdel='0'";
            string str2 = "select count(A.id) from [qp_oa_AddWorkFlow] as [A] inner join [qp_oa_AddWorkFlowPicRy] as [C] on [A].[Number] = [C].KeyFile  and [A].[UpNodeNum] = [C].xuhao  and [A].[GlNum] = [C].GlNum  where  BLusername='" + this.Session["username"] + "' and States='已委托' and State='正在办理' and Ifdel='0'";
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "ShanChu")
            {
                ...
                string sql = "select Number,UpNodeNum,UpNodeName,FormName,Name from qp_oa_AddWorkFlow where id='" + num + "'";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    string str2 = string.Concat(new object[] { "select id,IfZb,WtUsername from qp_oa_AddWorkFlowPicRy where KeyFile='", list["Number"].ToString(), "' and BLusername='", this.Session["username"], "' and XuHao='", list["UpNodeNum"], "'" });
                    SqlDataReader reader2 = this.List.GetList(str2);
                    if (reader2.Read())
                    {
                        string str3 = string.Concat(new object[] { "  Delete from qp_oa_AddWorkFlowPicRy  where KeyFile='", list["Number"].ToString(), "' and BLusername='", reader2["WtUsername"].ToString(), "' and XuHao='", list["UpNodeNum"], "'" });
                        this.List.ExeSql(str3);
                        string str4 = "Update qp_oa_AddWorkFlowPicRy  Set States='办理中',EndTime='',WtUsername='',WtRealname=''  where id='" + reader2["id"].ToString() + "'";
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                button.Attributes.Add("onclick", "javascript:return confirm('确定取消委托，收回工作[" + e.Row.Cells[0].Text.ToString() + "]，重新办理吗？')");
