namespace xyoa.pda.InfoManage.filesend
{
    ...
    public class JsInfoSend : Page
    {
        ...
        public string CreateMidSql()
        {
            ...
            {
                str = str + " and Realname like '%" + this.pulicss.GetFormatStr(this.FsRealname.Text) + "%'";
            }
        ...
        {
            string sql = string.Concat(new object[] { "select id,Name,Filetype,Settime,Username,Realname from qp_oa_InfoSend where CHARINDEX(',", this.Session["username"], ",',','+JsUsername+',') > 0 ", this.CreateMidSql(), " order by id desc" });
