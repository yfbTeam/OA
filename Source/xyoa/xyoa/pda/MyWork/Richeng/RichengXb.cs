namespace xyoa.pda.MyWork.Richeng
{
    ...
    public class RichengXb : Page
    {
        ...
        {
            string sql = string.Concat(new object[] { "select id,titles,StartTime,EndTime,Username,Gongkai,Realname from qp_oa_MyRicheng  where (CHARINDEX(',", this.Session["username"], ",',','+XbUsername+',') > 0) ", this.CreateMidSql(), " order by id desc" });
        ...
        {
            base.Response.Redirect("RichengXb.aspx");
        }
