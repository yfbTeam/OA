namespace xyoa.pda.WorkFlow
{
    ...
    public class WorkFlowListAll : Page
    {
        ...
        {
            string sql = string.Concat(new object[] { "select A.id,A.UpTimeSet,A.Shuxing,A.Number,A.JinJiChengDu,A.Sequence,A.FormName,A.Name,A.FlowNumber,A.FqUsername,A.FqRealname,A.NodeName,A.UpNodeName,A.UpNodeId,A.FormId,C.StartTime as SjStartTime,States  from [qp_oa_AddWorkFlow] as [A] inner join [qp_oa_AddWorkFlowPicRy] as [C] on [A].[Number] = [C].KeyFile and [A].[UpNodeNum] = [C].xuhao and [A].[GlNum] = [C].GlNum where BLusername='", this.Session["username"], "' and ((States='未接收' and State='正在办理') or (States='办理中' and State='正在办理') or (States='已办结' and State='正在办理') or (States='已委托' and State='正在办理')) and Ifdel='0' ", this.CreateMidSql(), " order by UpTimeSet desc" });
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                string sql = "select id,name,UpNodeId,FlowNumber,FormId,Number,UpNodeName,Shuxing,FormName,UpNodeNum,GlNum,NodeSite from qp_oa_AddWorkFlow  where id='" + label.Text + "'";
                ...
                if (list.Read())
                {
                    ...
                    {
                        if ((label11.Text.ToString() == "办理中") || (label11.Text.ToString() == "未接收"))
                        {
                            str2 = "";
                        ...
                        else if (label11.Text.ToString() == "已办结")
                        {
                            label2.Text = "";
                            label3.Text = "自由流程";
                            label8.Text = "";
                    ...
                    else if ((label11.Text.ToString() != "办理中") && !(label11.Text.ToString() == "未接收"))
                    {
                        if (label11.Text.ToString() == "已办结")
                        {
                            label2.Text = "";
