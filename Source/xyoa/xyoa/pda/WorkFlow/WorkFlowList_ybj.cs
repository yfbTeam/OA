namespace xyoa.pda.WorkFlow
{
    ...
    public class WorkFlowList_ybj : Page
    {
        ...
        {
            string sql = string.Concat(new object[] { "select A.id,A.UpTimeSet,A.Shuxing,A.Number,A.JinJiChengDu,A.Sequence,A.FormName,A.Name,A.FlowNumber,A.FqUsername,A.FqRealname,A.NodeName,A.UpNodeName,A.UpNodeId,A.FormId,C.StartTime as SjStartTime,States  from [qp_oa_AddWorkFlow] as [A] inner join [qp_oa_AddWorkFlowPicRy] as [C] on [A].[Number] = [C].KeyFile and [A].[UpNodeNum] = [C].xuhao and [A].[GlNum] = [C].GlNum where BLusername='", this.Session["username"], "' and States='已办结' and State='正在办理' and Ifdel='0' ", this.CreateMidSql(), " order by UpTimeSet desc" });
