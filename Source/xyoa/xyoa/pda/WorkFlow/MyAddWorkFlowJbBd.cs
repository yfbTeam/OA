namespace xyoa.pda.WorkFlow
{
    ...
    public class MyAddWorkFlowJbBd : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                string sql = "select * from qp_oa_AddWorkFlowGc where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'";
                ...
                list.Close();
                string str2 = "select UpNodeName,FlowNumber,Shuxing,FormNumber from qp_oa_AddWorkFlow where  Number='" + this.Number.Text + "'";
                ...
                if (reader2.Read())
                {
                    ...
                    while (reader3.Read())
                    {
                        ...
                        object obj2 = bag["url"];
                        (bag = this.ViewState)["url"] = string.Concat(new object[] { obj2, "window.L", reader3["Number"], ".location.href='/WorkFlow/AddWorkFlow_spyj.aspx?Number='+num+'&Buzhou=", reader3["sqlstr"], "&key=1';" });
                    }
                    reader3.Close();
                }
                reader2.Close();
