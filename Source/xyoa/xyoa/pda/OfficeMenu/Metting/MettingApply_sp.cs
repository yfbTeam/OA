namespace xyoa.pda.OfficeMenu.Metting
{
    ...
    public class MettingApply_sp : Page
    {
        ...
        {
            string sql = string.Concat(new object[] { "select A.* from [qp_oa_MettingApply] as [A]  where CHARINDEX(',", this.Session["username"], ",',','+DqBlUsername+',')   >0  and (LcZhuangtai='1' or LcZhuangtai='2') ", this.CreateMidSql(), " order by id desc" });
