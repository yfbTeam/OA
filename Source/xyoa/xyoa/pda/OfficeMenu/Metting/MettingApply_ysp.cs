namespace xyoa.pda.OfficeMenu.Metting
{
    ...
    public class MettingApply_ysp : Page
    {
        ...
        {
            string sql = string.Concat(new object[] { "select A.* from [qp_oa_MettingApply] as [A]  where CHARINDEX(',", this.Session["username"], ",',','+YjbUser+',')   >0 ", this.CreateMidSql(), " order by id desc" });
