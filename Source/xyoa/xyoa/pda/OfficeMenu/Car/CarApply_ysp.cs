namespace xyoa.pda.OfficeMenu.Car
{
    ...
    public class CarApply_ysp : Page
    {
        ...
        {
            string sql = string.Concat(new object[] { "select A.*, C.CarName from [qp_oa_CarApply] as [A] join [qp_oa_CarInfo] as [C] on [A].[CarId] = [C].id where CHARINDEX(',", this.Session["username"], ",',','+YjbUser+',')   >0 ", this.CreateMidSql(), " order by A.id desc" });
