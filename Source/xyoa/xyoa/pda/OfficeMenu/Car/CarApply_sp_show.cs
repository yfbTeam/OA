namespace xyoa.pda.OfficeMenu.Car
{
    ...
    public class CarApply_sp_show : Page
    {
        ...
        public void BindLb(string str)
        {
            ...
            if (list.Read())
            {
                ...
                else
                {
                    ...
                    {
                        this.ZbRealname.Attributes["onclick"] = "javascript:return openuser1();";
                    }
                }
            }
            list.Close();
        ...
        protected void Button3_Click(object sender, EventArgs e)
        {
            ...
            {
                string str2;
                str = string.Concat(new object[] { "Update qp_oa_CarApply  Set YjbUser=YjbUser+'", this.Session["username"], ",',DqBlUsername=replace(DqBlUsername,'", this.Session["username"].ToString(), ",','') where id='", int.Parse(base.Request.QueryString["id"]), "'" });
                ...
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='CarApply_sp.aspx'</script>");
            }
            else if (this.ZbRealname.Text != "")
            {
                ...
                while (list.Read())
                {
                    this.pulicss.InsertMessage("您有车辆使用需要审批，车辆名称：" + this.CarId.Text + "", list["username"].ToString(), list["realname"].ToString(), "/OfficeMenu/Car/CarApply.aspx");
                }
                list.Close();
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='CarApply_sp.aspx'</script>");
            }
        ...
        protected void Button4_Click(object sender, EventArgs e)
        {
            ...
            {
                string str2;
                str = string.Concat(new object[] { "Update qp_oa_CarApply  Set DqBlUsername=replace(DqBlUsername,'", this.Session["username"].ToString(), ",','') where id='", int.Parse(base.Request.QueryString["id"]), "'" });
                this.List.ExeSql(str);
                if (this.Yijian.Text.Trim() != "")
                {
                    str2 = string.Concat(new object[] { 
            ...
            else
            {
                ...
                this.pulicss.SpUpdateLog(this.Number.Text, this.Yijian.Text, "3", this.GlNum.Text, this.CaoZuo.SelectedValue, DateTime.Now.ToString());
                str = string.Concat(new object[] { "Update qp_oa_CarApply  Set  State='2',DqNodeId='',DqBlUsername='已结束',DqBlXinming='已结束',LcZhuangtai='3',YjbUser=YjbUser+'", this.Session["username"], ",',GlNum='", str3, "',YjbNodeId=YjbNodeId+'", this.YjbNodeId.Value, ",',DqNodeName='已结束',SNodeId='", this.YjbNodeId.Value, "' where id='", int.Parse(base.Request.QueryString["id"]), "'  " });
                this.List.ExeSql(str);
                this.pulicss.InsertMessage("您有车辆，名称[" + this.CarId.Text + "]已审批完成", this.Username.Text, this.Realname.Text, "/OfficeMenu/Car/CarApply.aspx");
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='CarApply_sp.aspx'</script>");
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                this.Button3.Attributes["onclick"] = "javascript:return LoadingShow();";
                ...
                if (list.Read())
                {
                    ...
                    this.State.Text = list["State"].ToString().Replace("1", "待审批").Replace("2", "已通过").Replace("3", "未通过").Replace("4", "使用中").Replace("5", "已结束").Replace("6", "终止审批");
        ...
        {
            int num = 0;
            string sql = "select DqBlUsername from qp_oa_CarApply   where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'";
            SqlDataReader list = this.List.GetList(sql);
            if (list.Read())
            {
