namespace xyoa.SystemManage.Juese
{
    ...
    public class Juese_mk3 : Page
    {
        ...
        private void BanmianGet()
        {
            ...
            reader5.Close();
            this.ViewState["bmstr_other"] = this.ViewState["bmstr2"].ToString() + this.ViewState["bmstr1"].ToString() + this.ViewState["bmstr4"].ToString() + this.ViewState["bmstr5"].ToString();
            ...
            for (int i = 0; i < this.GridView1.Rows.Count; i++)
            {
                ...
                if (this.pulicss.StrIFInStr("," + label.Text + ",", "," + this.ViewState["bmstr3"].ToString() + ",") && (label2.Text != "0"))
                {
                    box.Checked = true;
                }
            }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            string sql = string.Concat(new object[] { "Update qp_oa_Juese   Set Banmian3='", this.ViewState["s_id"].ToString(), "'   where id='", int.Parse(base.Request.QueryString["id"]), "'" });
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("修改角色", "角色管理");
            base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='Juese_mk3.aspx?id=" + int.Parse(base.Request.QueryString["id"]) + "'</script>");
        }
        ...
        {
            string sql = "select Banmian3 from qp_oa_Juese  where id='" + this.JueseId.SelectedValue + "'";
            SqlDataReader list = this.List.GetList(sql);
            if (list.Read())
            {
                string str2 = string.Concat(new object[] { "Update qp_oa_Juese   Set Banmian3='", list["Banmian3"].ToString(), "'  where id='", int.Parse(base.Request.QueryString["id"]), "'" });
