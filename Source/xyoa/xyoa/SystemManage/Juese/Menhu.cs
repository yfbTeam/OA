namespace xyoa.SystemManage.Juese
{
    ...
    public class Menhu : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            string sql = string.Concat(new object[] { "Update qp_oa_Juese  Set Mh='", this.Banmian.SelectedValue, "',MhBanmian='", this.ViewState["s_id"].ToString(), "'  where id='", int.Parse(base.Request.QueryString["id"]), "'" });
        ...
        public void DataBindToGridview()
        {
            ...
            string sql = "select id,name from qp_oa_Menhu order by Paixun asc";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                this.BindAttribute();
                string sql = "select Mh,MhBanmian from qp_oa_Juese where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    this.Banmian.SelectedValue = list["Mh"].ToString();
                    this.ViewState["PerSessionStr"] = list["MhBanmian"].ToString();
                }
                list.Close();
