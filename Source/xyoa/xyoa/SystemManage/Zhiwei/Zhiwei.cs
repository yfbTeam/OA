namespace xyoa.SystemManage.Zhiwei
{
    ...
    public class Zhiwei : Page
    {
        ...
        {
            base.Response.Redirect("Zhiwei_add.aspx");
        }
        ...
        {
            this.pulicss.QuanXianVis(this.AddData, "iiii8a", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.UpdateData, "iiii8c", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.ShowData, "iiii8b", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.DelData, "iiii8d", this.Session["perstr"].ToString());
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select * from qp_oa_Zhiwei where 1=1";
            string str2 = "select count(id) from qp_oa_Zhiwei where 1=1";
        ...
        protected void DelData_Click(object sender, EventArgs e)
        {
            ...
            string sql = "select * from qp_oa_username  where   Zhiweiid in (" + str + ")";
            SqlDataReader list = this.List.GetList(sql);
            if (list.Read())
            {
                base.Response.Write("<script language=javascript>alert('删除失败，其中存在正在使用的职位，请在用户管理中查看！');</script>");
            }
            else
            {
                list.Close();
                string str3 = "  Delete from qp_oa_Zhiwei  where ID in (" + str + ")";
                this.List.ExeSql(str3);
                this.pulicss.InsertLog("删除职位[" + this.CheckCbxNameDel() + "]", "职位管理");
                this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
            }
        }
        ...
        {
            base.Response.Redirect("Zhiwei_show.aspx?id=" + this.CheckCbxUpdate() + "");
        }

        protected void UpdateData_Click(object sender, EventArgs e)
        {
            base.Response.Redirect("Zhiwei_update.aspx?id=" + this.CheckCbxUpdate() + "");
        }
    }
}
