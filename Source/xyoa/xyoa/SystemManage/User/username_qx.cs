namespace xyoa.SystemManage.User
{
    ...
    public class username_qx : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            string sql = string.Concat(new object[] { "Update qp_oa_username  Set ResponQx='", str, "' where id='", int.Parse(base.Request.QueryString["id"]), "'" });
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("设置用户权限[" + this.Realname.Text + "]", "用户管理");
            base.Response.Write("<script language=javascript>alert('提交成功，您设置的用户，需要重新登陆后才能生效！');window.location.href='username.aspx'</script>");
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                if (list.Read())
                {
                    ...
                    this.ViewState["PerSessionStr"] = list["ResponQx"].ToString();
                }
                list.Close();
                for (int i = 0; i < this.GridView1.Rows.Count; i++)
                {
