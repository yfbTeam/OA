namespace xyoa.SystemManage.WorkFlow
{
    ...
    public class WorkFlowName_show_add_node_yz : Page
    {
        ...
        protected RadioButtonList YzTiaojian;
            ...
            if (this.TargetListBox.Items.Count > 0)
            {
                for (int i = 0; i <= (this.TargetListBox.Items.Count - 1); i++)
                {
                    ...
                    for (int j = 0; j < strArray.Length; j++)
                    {
                        ...
                        string sql = "select  *  from qp_oa_WorkFlowNameYz  where id='" + strArray[j] + "'";
                ...
                str4 = string.Concat(new object[] { "Update qp_oa_WorkFlowNode  Set YzTiaojian='", this.YzTiaojian.SelectedValue, "',YzFileID='", str, "' where id='", int.Parse(base.Request.QueryString["id"]), "'" });
                this.List.ExeSql(str4);
            }
            else
            {
                str4 = "Update qp_oa_WorkFlowNode  Set YzTiaojian='0',YzFileID='' where id='" + int.Parse(base.Request.QueryString["id"]) + "'";
                this.List.ExeSql(str4);
            }
        ...
        public void DataBindToDownList()
        {
            ...
            str = str + "'0'";
            string sQL = "select id,Name from qp_oa_WorkFlowNameYz   where  id in (" + str + ")  and FlowNumber='" + this.FlowNumber.Text + "' ";
            this.list.Bind_DropDownList_ListBox(this.TargetListBox, sQL, "id", "Name");
            string str4 = "select id,Name from qp_oa_WorkFlowNameYz   where  id not in (" + str + ") and FlowNumber='" + this.FlowNumber.Text + "'";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                if (list.Read())
                {
                    ...
                    this.WriteFileID.Text = list["YzFileID"].ToString();
                    this.YzTiaojian.SelectedValue = list["YzTiaojian"].ToString();
                }
                list.Close();
