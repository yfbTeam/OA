    ...
    public class FlowMg_left : Page
    {
        ...
        private void BindTree(TreeNodeCollection Nds)
        {
            ...
            while (list.Read())
            {
                ...
                while (reader2.Read())
                {
                    ...
                    node2.Text = string.Concat(new object[] { " <a href='FlowMg_list.aspx?FormId=", num2, "' target='nextrform'>", reader2["FormName"].ToString(), "</a>" });
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string sql = "select id from qp_oa_FormType";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    this.showtitle = "";
