namespace xyoa.SystemManage.WorkFlow
{
    ...
    public class WorkFlowName_show_add_node_btzd : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (this.TargetListBox.Items.Count > 0)
            {
                ...
                str4 = string.Concat(new object[] { "Update qp_oa_WorkFlowNode  Set CouFileID='", str, "',CouFileName='", str2, "' where id='", int.Parse(base.Request.QueryString["id"]), "'" });
                this.List.ExeSql(str4);
            }
            else
            {
                str4 = "Update qp_oa_WorkFlowNode  Set CouFileID='',CouFileName='' where id='" + int.Parse(base.Request.QueryString["id"]) + "'";
                this.List.ExeSql(str4);
            }
        ...
        public void DataBindToDownList()
        {
            ...
            str = str + "'0'";
            string sQL = string.Concat(new object[] { "select id,Name from qp_oa_FormFile   where  id in (", str, ")  and KeyFile='", this.FormNumber.Text, "' and (type='[常规型]' or type='[数字型]') and id  in (", this.ViewState["Kexie"], ")" });
            this.list.Bind_DropDownList_ListBox(this.TargetListBox, sQL, "id", "Name");
            string str4 = string.Concat(new object[] { "select id,Name from qp_oa_FormFile   where  id not in (", str, ") and KeyFile='", this.FormNumber.Text, "' and (type='[常规型]' or type='[数字型]')  and id  in (", this.ViewState["Kexie"], ")" });
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                if (list.Read())
                {
                    ...
                    this.WriteFileID.Text = list["CouFileID"].ToString();
