namespace xyoa.SystemManage.WorkFlow
{
    ...
    public class DIYForm_left : Page
    {
        ...
        private void BindTree(TreeNodeCollection Nds)
        {
            ...
            while (list.Read())
            {
                ...
                child.Text = string.Concat(new object[] { " <a href='DIYForm_show.aspx?id=", num, "' target='nextrform'>", list["Name"].ToString(), "</a>" });
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                this.pulicss.QuanXianVis(this.ListTreeView, "iiii1a", this.Session["perstr"].ToString());
                string sql = "select * from qp_oa_FormType";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    this.showtitle = "";
