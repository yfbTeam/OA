namespace xyoa.SystemManage.WorkFlow
{
    ...
    public class WorkFlowName_show_add_node_zj : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                reader3.Close();
                string str6 = null;
                str6 = "<vml:line title=\"\" style=\"DISPLAY: none; Z-INDEX: 2; POSITION: absolute\" to=\"0,0\" from=\"0,0\" coordsize=\"21600,21600\" arcsize=\"4321f\" object=\"" + this.NodeNum.Text + "\" source=\"" + this.shangbu.Text + "\" mfrID=\"" + this.shangbu.Text + "\"><vml:stroke endarrow=\"block\"></vml:stroke><vml:shadow offset=\"1px,1px\" color=\"#b3b3b3\" type=\"single\" on=\"T\"></vml:shadow></vml:line>";
                string str7 = "insert into qp_oa_WorkFlowNodeLine (Source,Object,LineContent,NodeNumber,FlowNumber) values ('" + this.shangbu.Text + "','" + this.NodeNum.Text + "','" + str6 + "','" + this.pulicss.GetFormatStr(this.NodeNumber.Text) + "','" + this.pulicss.GetFormatStr(this.FlowNumber.Text) + "')";
                ...
                if (this.TargetListBox.Items.Count > 0)
                {
                    for (int i = 0; i <= (this.TargetListBox.Items.Count - 1); i++)
                    {
                        ...
                        for (int j = 0; j < strArray.Length; j++)
                        {
                            ...
                            string str8 = "insert into qp_oa_WorkFlowNodeLine (Source,Object,LineContent,NodeNumber,FlowNumber) values ('" + this.NodeNum.Text + "','" + strArray[j] + "','" + str3 + "','" + this.pulicss.GetFormatStr(this.NodeNumber.Text) + "','" + this.pulicss.GetFormatStr(this.FlowNumber.Text) + "')";
                ...
                string str10 = "Update qp_oa_WorkFlowNode  Set UpNodeNum=UpNodeNum+'" + this.pulicss.GetFormatStr(this.NodeNum.Text) + ",'  where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "'";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                if (list.Read())
                {
                    this.shangbu.Text = list["NodeNum"].ToString();
                }
                list.Close();
                string str2 = "select top 1 *  from qp_oa_WorkFlowNode where FlowNumber='" + this.pulicss.GetFormatStr(base.Request.QueryString["FlowNumber"]) + "' order by NodeNum desc";
                SqlDataReader reader2 = this.List.GetList(str2);
                if (reader2.Read())
                {
                    this.NodeSite.SelectedValue = "中间段";
                    this.NodeNum.Text = this.NodeNum.Text + (int.Parse(reader2["NodeNum"].ToString()) + 1);
                    this.FormId.Text = reader2["FormId"].ToString();
                    this.FormNumber.Text = reader2["FormNumber"].ToString();
                    this.FormName.Text = reader2["FormName"].ToString();
                    this.FlowId.Text = reader2["FlowId"].ToString();
                ...
                else
                {
                    ...
                    this.NodeNum.Text = "1";
                    string str3 = "select  *  from qp_oa_WorkFlowName where FlowNumber='" + this.pulicss.GetFormatStr(base.Request.QueryString["FlowNumber"]) + "'";
                    SqlDataReader reader3 = this.List.GetList(str3);
                    if (reader3.Read())
                    {
                        this.FormId.Text = reader3["FormId"].ToString();
                        string str4 = "select  Number,FormName  from qp_oa_DIYForm where id='" + this.FormId.Text + "'";
                        SqlDataReader reader4 = this.List.GetList(str4);
                        if (reader4.Read())
                        {
                            this.FormNumber.Text = reader4["Number"].ToString();
                            this.FormName.Text = reader4["FormName"].ToString();
                        }
                        reader4.Close();
                        this.FlowId.Text = reader3["id"].ToString();
                        this.FlowNumber.Text = reader3["FlowNumber"].ToString();
                        this.FlowName.Text = reader3["FlowName"].ToString();
                    }
                    reader3.Close();
                }
                reader2.Close();
                ...
                this.NodeNumber.Text = "" + DateTime.Now.Year.ToString() + "" + DateTime.Now.Month.ToString() + "" + DateTime.Now.Day.ToString() + "" + DateTime.Now.Hour.ToString() + "" + DateTime.Now.Minute.ToString() + "" + DateTime.Now.Second.ToString() + "" + DateTime.Now.Millisecond.ToString() + "" + str5 + "";
            }
        }
    }
}
