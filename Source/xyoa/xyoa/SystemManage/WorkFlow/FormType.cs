namespace xyoa.SystemManage.WorkFlow
{
    ...
    public class FormType : Page
    {
        ...
        {
            base.Response.Redirect("FormType_add.aspx");
        }
        ...
        {
            this.pulicss.QuanXianVis(this.AddData, "iiii1a", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.GridView1, "iiii1a", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.UpdateData, "iiii1a", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.DelData, "iiii1a", this.Session["perstr"].ToString());
        }
        ...
        public void DataBindToGridview(string Sqlsort)
        {
            ...
            string sql = "select count(id) from  qp_oa_FormType";
            this.CountsLabel.Text = "" + this.List.GetCount(sql) + "";
            string str2 = "select id,Name,Paixun from qp_oa_FormType " + Sqlsort + "";
        ...
        protected void DelData_Click(object sender, EventArgs e)
        {
            ...
            string sql = "  Delete from qp_oa_FormType  where ID in (" + str + ")";
            this.List.ExeSql(sql);
            string str3 = "select id from qp_oa_DIYForm where TypeId in (" + str + ")";
            SqlDataReader list = this.List.GetList(str3);
            while (list.Read())
            {
                string str4 = "  Delete from qp_oa_DIYForm  where id='" + list["id"] + "'";
                this.List.ExeSql(str4);
                string str5 = "  Delete from qp_oa_WorkFlowName  where FormId='" + list["id"] + "'";
                this.List.ExeSql(str5);
            }
            list.Close();
            this.pulicss.InsertLog("删除表单类别", "表单类别");
            this.DataBindToGridview(this.SortText.Value);
        }
        ...
        {
            base.Response.Redirect("FormType_update.aspx?id=" + this.CheckCbxUpdate() + "");
        }
    }
}
