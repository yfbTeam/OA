namespace xyoa.OfficeMenu.Renwu
{
    ...
    public class RenwuFp : Page
    {
        ...
        {
            base.Response.Redirect("RenwuFp_add.aspx");
        }
        ...
        {
            this.pulicss.QuanXianVis(this.AddData, "kkkk7ba", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.UpdateData, "kkkk7bc", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.ShowData, "kkkk7bb", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.DelData, "kkkk7bd", this.Session["perstr"].ToString());
        }
        ...
        public string CreateMidSql()
        {
            ...
            {
                str = str + " and ZbUsername = '" + this.pulicss.GetFormatStr(this.ZbUsername.Text) + "'";
            }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.WcTime,A.Benbi,A.id,A.titles,A.Starttime,A.Endtime,A.ZbUsername,A.ZbRealname,A.Leixing,A.Benbi,A.State,A.Username,A.Realname,A.SetTime, B.[name] as LeiBieName from [qp_oa_Renwu] as [A] inner join [qp_oa_RenwuLx] as [B] on [A].[Leixing] = [B].[Id]  where username='" + this.Session["username"] + "'";
            string str2 = "select count(A.id)  from [qp_oa_Renwu] as [A] inner join [qp_oa_RenwuLx] as [B] on [A].[Leixing] = [B].[Id]  where username='" + this.Session["username"] + "'";
        ...
        protected void DelData_Click(object sender, EventArgs e)
        {
            ...
            string sql = "  Delete from qp_oa_Renwu  where ID in (" + str + ")";
            this.List.ExeSql(sql);
            string str3 = "  Delete from qp_oa_RenwuXb  where Keyid in (" + str + ")";
            this.List.ExeSql(str3);
            this.pulicss.InsertLog("删除任务", "任务分配");
            this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
        }
        ...
        {
            base.Response.Redirect("RenwuFp_show.aspx?id=" + this.CheckCbxUpdate() + "");
        }

        protected void UpdateData_Click(object sender, EventArgs e)
        {
            base.Response.Redirect("RenwuFp_update.aspx?id=" + this.CheckCbxUpdate() + "");
        }
    }
}
