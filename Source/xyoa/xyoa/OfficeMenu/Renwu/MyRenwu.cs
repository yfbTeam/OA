namespace xyoa.OfficeMenu.Renwu
{
    ...
    public class MyRenwu : Page
    {
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = string.Concat(new object[] { "select A.WcTime,A.Benbi,A.id,A.titles,A.Starttime,A.Endtime,A.ZbUsername,A.ZbRealname,A.Leixing,A.Benbi,A.State,A.Username,A.Realname,A.SetTime, B.[name] as LeiBieName from [qp_oa_Renwu] as [A] inner join [qp_oa_RenwuLx] as [B] on [A].[Leixing] = [B].[Id]  where (ZbUsername='", this.Session["username"], "' or (CHARINDEX(',", this.Session["username"], ",',','+JbUsername+',')   >   0 ))" });
            string str2 = string.Concat(new object[] { "select count(A.id)  from [qp_oa_Renwu] as [A] inner join [qp_oa_RenwuLx] as [B] on [A].[Leixing] = [B].[Id]  where (ZbUsername='", this.Session["username"], "' or (CHARINDEX(',", this.Session["username"], ",',','+JbUsername+',')   >   0 ))" });
