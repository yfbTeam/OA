namespace xyoa.OfficeMenu.Renwu
{
    ...
    public class RenwuFp_add : Page
    {
        ...
        {
            string sql = "insert into qp_oa_Renwu (YdUsername,YdRealname,WcTime,Number,titles,Content,Starttime,Endtime,ZbUsername,ZbRealname,JbUsername,JbRealname,Leixing,Benbi,State,Username,Realname,SetTime) values ('','','2000-01-01','" + this.pulicss.GetFormatStr(this.Number.Text) + "','" + this.pulicss.GetFormatStr(this.titles.Text) + "','" + this.pulicss.GetFormatStrbjq(this.Content.Value) + "','" + this.pulicss.GetFormatStr(this.Starttime.Text) + "','" + this.pulicss.GetFormatStr(this.Endtime.Text) + "','" + this.pulicss.GetFormatStr(this.ZbUsername.Text) + "','" + this.pulicss.GetFormatStr(this.ZbRealname.Text) + "','" + this.pulicss.GetFormatStr(this.JbUsername.Text) + "','" + this.pulicss.GetFormatStr(this.JbRealname.Text) + "','" + this.Leixing.SelectedValue + "','0','1','" + this.Session["Username"].ToString() + "','" + this.Session["Realname"].ToString() + "','" + DateTime.Now.ToString() + "')";
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("新增任务", "任务分配");
            base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='RenwuFp.aspx'</script>");
            if (this.C1.Checked)
            {
                this.pulicss.InsertMessage("您有新的任务需要参加：" + this.titles.Text + "", this.ZbUsername.Text, this.ZbRealname.Text, "/OfficeMenu/Renwu/MyRenwu.aspx");
            }
            ...
            string str3 = "select top 1 id from qp_oa_Renwu where username='" + this.Session["username"] + "' order by id desc";
            SqlDataReader reader2 = this.List.GetList(str3);
            if (reader2.Read())
            {
                this.ViewState["Keyid"] = reader2["id"].ToString();
            }
            reader2.Close();
            string str4 = null;
            string str5 = null;
            str5 = "" + this.JbUsername.Text + "";
            ...
            while (reader3.Read())
            {
                string str7 = string.Concat(new object[] { "insert into qp_oa_RenwuXb (Keyid,Username,Realname,Zhuangtai,Neirong,Baifenbi) values ('", this.ViewState["Keyid"], "','", reader3["username"].ToString(), "','", reader3["realname"].ToString(), "','1','','0')" });
                this.List.ExeSql(str7);
                if (this.CheckBox3.Checked)
                {
                    this.pulicss.InsertMessage("您有新的任务需要参加：" + this.titles.Text + "", reader3["username"].ToString(), reader3["realname"].ToString(), "/OfficeMenu/Renwu/MyRenwuXb.aspx");
                }
                if (this.CheckBox5.Checked)
                {
                    this.pulicss.InsertSms(reader3["MoveTel"].ToString(), "您有新的任务需要参加：" + this.titles.Text + "");
                }
            }
            reader3.Close();
        }
