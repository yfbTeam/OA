namespace xyoa.OfficeMenu.Car
{
    ...
    public class CarCosts : Page
    {
        ...
        {
            base.Response.Redirect("CarCosts_add.aspx");
        }

        public void BindAttribute()
        {
            this.CostsName.Attributes["onkeydown"] = "if (event.keyCode==13) { document.all.SearchData.click(); return false;}";
        ...
        {
            this.pulicss.QuanXianVis(this.AddData, "kkkk3fa", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.UpdateData, "kkkk3fc", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.ShowData, "kkkk3fb", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.DelData, "kkkk3fd", this.Session["perstr"].ToString());
        }
        ...
        {
            string str = string.Empty;
            if (this.CarName.SelectedValue != "")
            {
                str = str + " and CarName = '" + this.CarName.SelectedValue + "'";
            }
            if (this.BuMenId.SelectedValue != "")
            {
                str = str + " and BuMenId = '" + this.BuMenId.SelectedValue + "'";
            }
            if (this.CostsType.SelectedValue != "")
            {
                str = str + " and CostsType = '" + this.CostsType.SelectedValue + "'";
            }
            if (this.PersonName.Text != "")
            {
                str = str + " and PersonName like '%" + this.pulicss.GetFormatStr(this.PersonName.Text) + "%'";
            }
            if (this.CostsName.Text != "")
            {
                str = str + " and CostsName like '%" + this.pulicss.GetFormatStr(this.CostsName.Text) + "%'";
            }
            return str;
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.*, C.Name from [qp_oa_CarCosts] as [A] join [qp_oa_Bumen] as [C] on [A].[BuMenId] = [C].id where A.Username='" + this.Session["Username"] + "'";
            string str2 = "select count(id) from qp_oa_CarCosts where  Username='" + this.Session["Username"] + "'";
            string str3 = "select sum(CostsMoney) as summoney  from qp_oa_CarCosts where Username='" + this.Session["Username"] + "'";
        ...
        protected void DelData_Click(object sender, EventArgs e)
        {
            ...
            string sql = "  Delete from qp_oa_CarCosts  where ID in (" + str + ")";
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("删除车辆费用管理 ", "车辆费用管理 ");
            this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
        }
        ...
        {
            base.Response.Redirect("CarCosts_show.aspx?id=" + this.CheckCbxUpdate() + "");
        }

        protected void UpdateData_Click(object sender, EventArgs e)
        {
            base.Response.Redirect("CarCosts_update.aspx?id=" + this.CheckCbxUpdate() + "");
        }
    }
}
