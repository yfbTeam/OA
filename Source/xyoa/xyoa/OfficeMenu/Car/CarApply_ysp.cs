namespace xyoa.OfficeMenu.Car
{
    ...
    public class CarApply_ysp : Page
    {
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.*, C.CarName from [qp_oa_CarApply] as [A] join [qp_oa_CarInfo] as [C] on [A].[CarId] = [C].id where CHARINDEX('," + this.Session["username"] + ",',','+YjbUser+',')   >0 ";
            string str2 = "select count(A.id) from [qp_oa_CarApply] as [A] join [qp_oa_CarInfo] as [C] on [A].[CarId] = [C].id where CHARINDEX('," + this.Session["username"] + ",',','+YjbUser+',')   >0";
