namespace xyoa.OfficeMenu.WorkPlan
{
    ...
    public class MyPlanJkBm_left : Page
    {
        ...
        private void BindTree(TreeNodeCollection Nds, int IDStr)
        {
            ...
            while (list.Read())
            {
                ...
                if (IDStr == 0)
                {
                    ...
                    str2 = string.Concat(new object[] { "select username,realname from qp_oa_Username where  CHARINDEX(','+Username+',',','+(select RyUsername from qp_oa_MyPlanSz where ZgUsername='", this.Session["username"], "')+',') > 0  and BuMenId='", list["ID"].ToString(), "'   order by paixu asc" });
                    ...
                    while (reader2.Read())
                    {
                        ...
                        node2.Text = " <a href='MyPlanJk_list.aspx?user=" + reader2["username"].ToString() + "' target='nextrform'>" + reader2["realname"].ToString() + "</a>";
