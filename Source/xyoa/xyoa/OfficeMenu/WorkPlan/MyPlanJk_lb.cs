namespace xyoa.OfficeMenu.WorkPlan
{
    ...
    public class MyPlanJk_lb : Page
    {
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.Biaoti,A.Leixing,A.StartTime,A.EndTime,A.Youxian,A.DqState,A.SetTimes,A.Username,A.Realname, B.[Leixing] as LeixingName from [qp_oa_MyPlan] as [A] inner join [qp_oa_MyPlanLx] as [B] on [A].[Leixing] = [B].[Id]  where 1=1 and  CHARINDEX(','+A.Username+',',','+(select RyUsername from qp_oa_MyPlanSz where ZgUsername='" + this.Session["username"] + "')+',') > 0";
            string str2 = "select count(A.id) from [qp_oa_MyPlan] as [A] inner join [qp_oa_MyPlanLx] as [B] on [A].[Leixing] = [B].[Id] where 1=1 and  CHARINDEX(','+A.Username+',',','+(select RyUsername from qp_oa_MyPlanSz where ZgUsername='" + this.Session["username"] + "')+',') > 0";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                string sQL = "select username, Realname  from qp_oa_username  where 1=1 and ifdel='0' and CHARINDEX(','+Username+',',','+(select RyUsername from qp_oa_MyPlanSz where ZgUsername='" + this.Session["username"] + "')+',') > 0";
                this.list.Bind_DropDownList_CheckBoxList(this.CheckBoxList1, sQL, "username", "Realname");
                string str2 = "select id,Leixing  from qp_oa_MyPlanLx order by id asc";
                this.list.Bind_DropDownList_kong(this.Leixing, str2, "id", "Leixing");
                this.BindAttribute();
                this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
                this.Bindquanxian();
            }
