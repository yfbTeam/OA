namespace xyoa.OfficeMenu.WorkPlan
{
    ...
    public class MyPlanJk_list : Page
    {
        ...
        {
            this.pulicss.QuanXianVis(this.GridView1, "kkkk5c", this.Session["perstr"].ToString());
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            {
                base.Response.Redirect("MyPlanJk_list.aspx?user=" + base.Request.QueryString["user"] + "");
            }
            if (base.Request.QueryString["StartTime"] != null)
            {
                base.Response.Redirect("MyPlanJk_list.aspx?StartTime=" + base.Request.QueryString["StartTime"] + "&EndTime=" + base.Request.QueryString["EndTime"] + "");
            }
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.Biaoti,A.Leixing,A.StartTime,A.EndTime,A.Youxian,A.DqState,A.SetTimes,A.Username,A.Realname, B.[Leixing] as LeixingName from [qp_oa_MyPlan] as [A] inner join [qp_oa_MyPlanLx] as [B] on [A].[Leixing] = [B].[Id]  where 1=1 and CHARINDEX(','+Username+',',','+(select RyUsername from qp_oa_MyPlanSz where ZgUsername='" + this.Session["username"] + "')+',') > 0";
            string str2 = "select count(A.id) from [qp_oa_MyPlan] as [A] inner join [qp_oa_MyPlanLx] as [B] on [A].[Leixing] = [B].[Id] where 1=1 and CHARINDEX(','+Username+',',','+(select RyUsername from qp_oa_MyPlanSz where ZgUsername='" + this.Session["username"] + "')+',') > 0";
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                string sql = "select Biaoti from qp_oa_MyPlan where id='" + label.Text + "'  ";
                SqlDataReader list = this.List.GetList(sql);
                if (list.Read())
                {
                    if (base.Request.QueryString["user"] != null)
                    {
                        label2.Text = string.Concat(new object[] { "<a href='MyPlanJk_show.aspx?id=", label.Text, "&user=", base.Request.QueryString["user"], "' class=\"LinkLine\">", list["Biaoti"], "</a>" });
                    }
                    else if (base.Request.QueryString["StartTime"] != null)
                    {
                        label2.Text = string.Concat(new object[] { "<a href='MyPlanJk_show.aspx?id=", label.Text, "&StartTime=", base.Request.QueryString["StartTime"], "&EndTime=", base.Request.QueryString["EndTime"], "' class=\"LinkLine\">", list["Biaoti"], "</a>" });
                    }
                    else
                    {
                        label2.Text = string.Concat(new object[] { "<a href='MyPlanJk_show.aspx?id=", label.Text, "' class=\"LinkLine\">", list["Biaoti"], "</a>" });
                    }
                }
                list.Close();
            }
