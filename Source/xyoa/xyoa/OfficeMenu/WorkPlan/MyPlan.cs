namespace xyoa.OfficeMenu.WorkPlan
{
    ...
    public class MyPlan : Page
    {
        ...
        {
            base.Response.Redirect("MyPlan_add.aspx");
        }
        ...
        public void Bindquanxian()
        {
            ...
            this.pulicss.QuanXianVis(this.AddData, "kkkk5aa", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.UpdateData, "kkkk5ac", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.ShowData, "kkkk5ab", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.DelData, "kkkk5ad", this.Session["perstr"].ToString());
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select A.id,A.Biaoti,A.Leixing,A.StartTime,A.EndTime,A.Youxian,A.DqState,A.SetTimes,A.Username,A.Realname, B.[Leixing] as LeixingName from [qp_oa_MyPlan] as [A] inner join [qp_oa_MyPlanLx] as [B] on [A].[Leixing] = [B].[Id]  where 1=1 and Username='" + this.Session["Username"] + "'";
            string str2 = "select count(A.id) from [qp_oa_MyPlan] as [A] inner join [qp_oa_MyPlanLx] as [B] on [A].[Leixing] = [B].[Id] where 1=1 and Username='" + this.Session["Username"] + "'";
        ...
        protected void DelData_Click(object sender, EventArgs e)
        {
            ...
            string sql = "  Delete from qp_oa_MyPlan  where ID in (" + str + ")";
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("删除工作计划", "工作计划");
            this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
        }
        ...
        {
            base.Response.Redirect("MyPlan_show.aspx?id=" + this.CheckCbxUpdate() + "");
        }

        protected void UpdateData_Click(object sender, EventArgs e)
        {
            base.Response.Redirect("MyPlan_update.aspx?id=" + this.CheckCbxUpdate() + "");
        }
    }
}
