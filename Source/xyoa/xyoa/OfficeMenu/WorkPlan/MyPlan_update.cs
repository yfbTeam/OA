namespace xyoa.OfficeMenu.WorkPlan
{
    ...
    public class MyPlan_update : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            string sql = string.Concat(new object[] { 
                "Update qp_oa_MyPlan  Set Biaoti='", this.pulicss.GetFormatStr(this.Biaoti.Text), "',Leixing='", this.Leixing.SelectedValue, "',Neirong='", this.pulicss.GetFormatStrbjq(this.Neirong.Value), "',StartTime='", this.pulicss.GetFormatStr(this.StartTime.Text), "',EndTime='", this.pulicss.GetFormatStr(this.EndTime.Text), "',Youxian='", this.Youxian.SelectedValue, "',DqState='", this.DqState.SelectedValue, "',Sharing='", this.Sharing.SelectedValue, 
                "',SharingNameZd='", this.pulicss.GetFormatStr(this.SharingNameZd.Text), "',SharingUserZd='", this.pulicss.GetFormatStr(this.SharingUserZd.Text), "',SharingNameKx='", this.pulicss.GetFormatStr(this.SharingNameKx.Text), "',SharingUserKx='", this.pulicss.GetFormatStr(this.SharingUserKx.Text), "'  where id='", int.Parse(base.Request.QueryString["id"]), "' "
             });
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!this.Page.IsPostBack)
                {
                    ...
                    string sql = "select * from qp_oa_MyPlan  where id='" + this.pulicss.GetFormatStr(base.Request.QueryString["id"]) + "' ";
                    ...
                    if (list.Read())
                    {
                        ...
                        this.SharingUserZd.Text = list["SharingUserZd"].ToString();
                        this.SharingNameKx.Text = list["SharingNameKx"].ToString();
                        this.SharingUserKx.Text = list["SharingUserKx"].ToString();
                    }
                    list.Close();
                    this.BindAttribute();
                }
