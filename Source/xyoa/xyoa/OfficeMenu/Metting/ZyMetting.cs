namespace xyoa.OfficeMenu.Metting
{
    ...
    public class ZyMetting : Page
    {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (str != null)
                    {
                        ...
                        {
                            sql = "select id,LcZhuangtai,Starttime,Endtime,MettingName,Realname from qp_oa_MettingApply  where datediff(dd,Starttime,'" + e.Day.Date.ToShortDateString() + "')=0";
                        }
                        else
                        {
                            sql = "select id,LcZhuangtai,Starttime,Endtime,MettingName,Realname from qp_oa_MettingApply  where datediff(dd,Starttime,'" + e.Day.Date.ToShortDateString() + "')=0 and (MettingId='" + this.typename.SelectedValue + "')";
                        }
                        ...
                        while (list.Read())
                        {
                            ...
                            cell1.Text = text + "<img src=\"/oaimg/zy/" + list["LcZhuangtai"].ToString() + ".gif\" border=0 hspace=2 alt=" + list["LcZhuangtai"].ToString().Replace("1", "等待办理").Replace("2", "正在办理").Replace("3", "通过审批").Replace("4", "终止审批") + ">" + list["Starttime"].ToString() + "至" + list["Endtime"].ToString() + "<font color=#666666>(" + list["Realname"].ToString() + "→" + list["MettingName"].ToString() + ")</font><br>";
                        }
                        list.Close();
        ...
        public void DataBindToGridview(string StrCheck)
        {
            ...
            if (StrCheck == "0")
            {
                str = "select Starttime from qp_oa_MettingApply";
            ...
            {
                str = "select Starttime from qp_oa_MettingApply where MettingId='" + StrCheck + "'";
                list = this.List.GetList(str);
                while (list.Read())
                {
                    this.a = int.Parse(DateTime.Parse(list["StartTime"].ToString()).Year.ToString());
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!this.Page.IsPostBack)
                {
                    ...
                    this.list.Bind_DropDownListMetting(this.typename, sQL, "id", "name");
