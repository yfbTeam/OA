namespace xyoa.web
{
    ...
    public class syTupian_view : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.Page.IsPostBack)
            {
                ...
                list.Close();
                string str2 = "select * from qp_web_Tupian where id='" + int.Parse(this.pulicss.GetFormatStr(base.Request.QueryString["id"])) + "'";
                SqlDataReader reader2 = this.List.GetList(str2);
                if (reader2.Read())
                {
                    this.photo = reader2["Tupian"].ToString();
                    this.settime = reader2["Shijian"].ToString();
                    this.title = reader2["Titles"].ToString();
                    this.content = reader2["content"].ToString();
                }
                reader2.Close();
                this.ViewState["LeftName"] = "图片新闻";
