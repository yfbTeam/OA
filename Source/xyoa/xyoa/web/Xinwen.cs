namespace xyoa.web
{
    ...
    public class Xinwen : Page
    {
        ...
        public void DataBindToGridview()
        {
            ...
            string sql = null;
            sql = "select count(id) from   qp_web_Xinwen  where 1=1 " + this.CreateMidSql() + "";
            ...
            {
                string str2 = null;
                str2 = string.Concat(new object[] { "select top 16 id,Titles,Shijian from qp_web_Xinwen  where id not in (select top ", this.pageno, " id  from qp_web_Xinwen where 1=1 ", this.CreateMidSql(), "  order by id desc) and  1=1 ", this.CreateMidSql(), "  order by id desc" });
                ...
                while (list.Read())
                {
                    ...
                    this.listall = listall + "<tr><td width=\"500\" height=\"36\" align=\"left\" background=\"img/diandian.jpg\"><b>\x00b7</b>&nbsp;<a href=\"Xinwen_view.aspx?id=" + list["id"].ToString() + "\">" + list["Titles"].ToString() + "</a></td><td width=\"100\" height=\"36\" background=\"img/diandian.jpg\" >[" + DateTime.Parse(list["Shijian"].ToString()).ToShortDateString() + "]</td></tr>";
                    num++;
                    if (num == 1)
                    {
                        num = 0;
