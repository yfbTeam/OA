namespace xyoa.web
{
    ...
    public class Yuandi : Page
    {
        ...
        public string CreateMidSql()
        {
            ...
            if ((base.Request.QueryString["Leibie"] != null) && (base.Request.QueryString["Leibie"] != "0"))
            {
                str = string.Concat(new object[] { str, " and Leibie='", int.Parse(base.Request.QueryString["Leibie"]), "'" });
            }
            return str;
        }
        ...
        public void DataBindToGridview()
        {
            ...
            string sql = null;
            sql = "select count(id) from   qp_web_Yuandi  where 1=1 " + this.CreateMidSql() + "";
            ...
            {
                string str2 = null;
                str2 = string.Concat(new object[] { "select top 16 id,Titles,Shijian,Leibie,NewName from qp_web_Yuandi  where id not in (select top ", this.pageno, " id  from qp_web_Yuandi where   1=1  ", this.CreateMidSql(), "  order by id desc) and   1=1   ", this.CreateMidSql(), "  order by id desc" });
                ...
                while (list.Read())
                {
                    ...
                    this.listall = listall + "<tr><td width=\"500\" height=\"36\" align=\"left\" background=\"img/diandian.jpg\"><b>\x00b7</b>&nbsp;<a href=\"Yuandi_down.aspx?NewName=" + list["NewName"].ToString() + "&Leibie=" + list["Leibie"].ToString() + "\" target=_blank>" + list["Titles"].ToString() + "</a></td><td width=\"100\" height=\"36\" background=\"img/diandian.jpg\" >[" + DateTime.Parse(list["Shijian"].ToString()).ToShortDateString() + "]</td></tr>";
                    num++;
                    if (num == 1)
                    {
                        num = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.Page.IsPostBack)
            {
                ...
                {
                    if (base.Request.QueryString["Leibie"] != "0")
                    {
                        this.ViewState["LeftName"] = this.pulicss.TypeCodeAll("Name", "qp_web_YuandiLb", this.pulicss.GetFormatStr(base.Request.QueryString["Leibie"].ToString()));
                        this.Right1.A_value = this.ViewState["LeftName"].ToString();
                    }
                    else
                    {
                        this.ViewState["LeftName"] = "学习园地";
                        this.Right1.A_value = "学习园地";
                    }
                }
                else
                {
