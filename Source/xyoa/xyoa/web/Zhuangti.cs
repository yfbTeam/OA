namespace xyoa.web
{
    ...
    public class Zhuangti : Page
    {
        ...
        public void DataBindToGridview()
        {
            ...
            string sql = null;
            sql = "select count(id) from   qp_web_Zhuanti  where 1=1 " + this.CreateMidSql() + "";
            ...
            {
                string str2 = null;
                str2 = string.Concat(new object[] { "select top 16 id,Titles,Shijian,Leibie from qp_web_Zhuanti  where id not in (select top ", this.pageno, " id  from qp_web_Zhuanti where   1=1  ", this.CreateMidSql(), "  order by id desc) and   1=1   ", this.CreateMidSql(), "  order by id desc" });
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.Page.IsPostBack)
            {
                ...
                if (base.Request.QueryString["Leibie"] != null)
                {
                    ...
                    {
                        this.ViewState["LeftName"] = this.pulicss.TypeCodeAll("Name", "qp_web_ZhuantiLb", this.pulicss.GetFormatStr(base.Request.QueryString["Leibie"].ToString()));
                        this.Right1.A_value = this.ViewState["LeftName"].ToString();
                    }
                    else
                    {
                        this.ViewState["LeftName"] = "专题报道";
                        this.Right1.A_value = "专题报道";
                    }
                }
                else
                {
