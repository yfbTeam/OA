        ...
        private void BindTree(TreeNodeCollection Nds, int IDStr)
        {
            ...
            while (list.Read())
            {
                ...
                if (IDStr == 0)
                {
                    ...
                    str2 = "select A.id,A.username, A.realname,A.lasttime,  C.[States], C.[Shuoming]  from [qp_oa_username] as [A]  inner join [qp_oa_MyState] as [C] on [A].[username] = [C].[username] and [A].[BuMenId]='" + list["ID"].ToString() + "' order by A.id asc";
