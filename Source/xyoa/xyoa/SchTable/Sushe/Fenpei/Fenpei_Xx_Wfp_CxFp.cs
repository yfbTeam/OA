namespace xyoa.SchTable.Sushe.Fenpei
{
    ...
    public class Fenpei_Xx_Wfp_CxFp : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                string sql = "select id,Sushe from qp_sch_Xuesheng where  id in (" + base.Request.QueryString["XsId"] + ")";
                SqlDataReader list = this.List.GetList(sql);
                while (list.Read())
                {
                    string str2 = "Update qp_sch_Sushe    Set Yiyong=Yiyong-1 where  id='" + list["Sushe"].ToString() + "'";
                    this.List.ExeSql(str2);
                    string str3 = "Update qp_sch_Sushe    Set Yiyong=Yiyong+1 where  id='" + this.Sushe.SelectedValue + "'";
                    this.List.ExeSql(str3);
                }
                list.Close();
                string str4 = "Update qp_sch_Xuesheng    Set Sushe='" + this.Sushe.SelectedValue + "' where  id in (" + base.Request.QueryString["XsId"] + ")";
                this.List.ExeSql(str4);
                this.pulicss.InsertLog("宿舍分配", "宿舍分配");
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='Fenpei_Xx_Yfp.aspx?id=" + base.Request.QueryString["id"] + "'</script>");
            }
        }

        protected void Button2_Click(object sender, EventArgs e)
        {
            base.Response.Redirect("Fenpei_Xx_Yfp.aspx?id=" + base.Request.QueryString["id"] + "");
        }
