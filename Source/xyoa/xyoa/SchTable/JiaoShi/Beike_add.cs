namespace xyoa.SchTable.JiaoShi
{
    ...
    public class Beike_add : Page
    {
        ...
        {
            string sql = "insert into qp_sch_Beike  (Xueqi,Xueduan,Number,titles,Content,Settime,Username,Realname) values ('" + this.Xueqi.SelectedValue + "','" + this.Xueduan.SelectedValue + "','" + this.pulicss.GetFormatStr(this.Number.Text) + "','" + this.pulicss.GetFormatStr(this.titles.Text) + "','" + this.pulicss.GetFormatStrbjq(this.Content.Value) + "','" + DateTime.Now.ToString() + "','" + this.Session["Username"].ToString() + "','" + this.Session["Realname"].ToString() + "')";
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("新增我的备课", "我的备课");
