namespace xyoa.SchTable.Xueji.Chaxun
{
    ...
    public class Chaxun_Bys : Page
    {
        protected DropDownList BanJi;
        protected TextBox BiYeId;
        ...
        {
            string sQL = "select C.id,C.Xingming  from [stu_table_" + this.Xueqi.SelectedValue + "_" + this.Xueduan.SelectedValue.Replace("上学期", "1").Replace("下学期", "2") + "] as [A] inner join [qp_sch_Xuesheng] as [C] on [A].[XsId] = [C].[Id] where  A.BanJi='" + this.BanJi.SelectedValue + "' and A.XuejiZhuangtai='" + this.BiYeId.Text + "' order by A.id asc";
        ...
        {
            string str = "select A.XsId from [stu_table_" + this.Xueqi.SelectedValue + "_" + this.Xueduan.SelectedValue.Replace("上学期", "1").Replace("下学期", "2") + "] as [A] inner join [qp_sch_Xuesheng] as [C] on [A].[XsId] = [C].[Id] inner join [qp_sch_Banji] as [B] on [A].[BanJi] = [B].[Id] where 1=1 and A.XuejiZhuangtai='" + this.BiYeId.Text + "'";
        ...
        protected void Nianji_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string str2 = "select C.id,C.Xingming  from [stu_table_" + this.Xueqi.SelectedValue + "_" + this.Xueduan.SelectedValue.Replace("上学期", "1").Replace("下学期", "2") + "] as [A] inner join [qp_sch_Xuesheng] as [C] on [A].[XsId] = [C].[Id] where  A.BanJi='" + this.BanJi.SelectedValue + "' and A.XuejiZhuangtai='" + this.BiYeId.Text + "' order by A.id asc";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (!this.Page.IsPostBack)
                {
                    ...
                    string sql = "select id from qp_sch_DataFile where  Name='毕业' and type='20'";
                    SqlDataReader list = this.List.GetList(sql);
                    if (list.Read())
                    {
                        this.BiYeId.Text = list["id"].ToString();
                    }
                    list.Close();
                    ...
                    string str3 = "select A.id,A.NianjiMc  from [qp_sch_Nianji] as [A] inner join [qp_sch_SetSclClass] as [C] on [A].[NianjiMc] = [C].[Name] where  1=1  order by Num asc";
                    this.list.Bind_DropDownList_nothing(this.Nianji, str3, "NianjiMc", "NianjiMc");
                    string str4 = string.Concat(new object[] { "select A.id,Mingcheng from [qp_sch_Banji] as [A] inner join [qp_sch_SetSclClass] as [C] on [A].[Nianji] = [C].[Name] where  ((CHARINDEX(',", this.Session["username"], ",',','+A.RkUsername+',') > 0) or (CHARINDEX(',", this.Session["username"], ",',','+A.XldUsername+',') > 0) or BzUsername='", this.Session["username"], "') order by Num asc" });
                    this.list.Bind_DropDownList_nothing(this.BanJi, str4, "id", "Mingcheng");
                    string str5 = "select C.id,C.Xingming  from [stu_table_" + this.Xueqi.SelectedValue + "_" + this.Xueduan.SelectedValue.Replace("上学期", "1").Replace("下学期", "2") + "] as [A] inner join [qp_sch_Xuesheng] as [C] on [A].[XsId] = [C].[Id] where  A.BanJi='" + this.BanJi.SelectedValue + "' and A.XuejiZhuangtai='" + this.BiYeId.Text + "' order by A.id asc";
                    this.list.Bind_DropDownList_kong(this.XsId, str5, "id", "Xingming");
                }
            }
        }
        ...
        protected void Xueqi_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string str3 = "select C.id,C.Xingming  from [stu_table_" + this.Xueqi.SelectedValue + "_" + this.Xueduan.SelectedValue.Replace("上学期", "1").Replace("下学期", "2") + "] as [A] inner join [qp_sch_Xuesheng] as [C] on [A].[XsId] = [C].[Id] where  A.BanJi='" + this.BanJi.SelectedValue + "' and A.XuejiZhuangtai='" + this.BiYeId.Text + "' order by A.id asc";
