namespace xyoa.SchTable.Xueji.Xueshengxinxi
{
    ...
    public class Xueshengxinxi_jtxx : Page
    {
        ...
        protected void AddData_Click(object sender, EventArgs e)
        {
            ...
            {
                base.Response.Redirect("Xueshengxinxi_jtxx_add.aspx?XsId=" + base.Request.QueryString["XsId"] + "&Xueduan=" + base.Request.QueryString["Xueduan"] + "&Xueqi=" + base.Request.QueryString["Xueqi"] + "");
            }
            else
            {
                base.Response.Redirect("Xueshengxinxi_jtxx_add.aspx?Banji=" + base.Request.QueryString["id"] + "&Xueduan=" + base.Request.QueryString["Xueduan"] + "&Xueqi=" + base.Request.QueryString["Xueqi"] + "");
            }
        }
        ...
        {
            this.pulicss.QuanXianVis(this.GridView1, "pppp2bab", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.AddData, "pppp2baa", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.UpdateData, "pppp2bac", this.Session["perstr"].ToString());
            this.pulicss.QuanXianVis(this.DelData, "pppp2bad", this.Session["perstr"].ToString());
        }
        ...
        public void DataBindToGridview(string Sqlsort, string SqlCreate)
        {
            ...
            string str = "select '" + this.divsch.GetXueduan().Replace("上学期", "1").Replace("下学期", "2") + "' as Xueduan,D.*,A.Xueqi,A.BanJi,A.XsId,A.Zhiwu,A.XuejiZhuangtai,C.Xingming as XsName,B.[Mingcheng] as BanJiName from [stu_table_" + this.divsch.GetXueqi() + "_" + this.divsch.GetXueduan().Replace("上学期", "1").Replace("下学期", "2") + "] as [A] inner join [qp_sch_Xuesheng] as [C] on [A].[XsId] = [C].[Id] inner join [qp_sch_Stu_jtxx] as [D] on [A].[XsId] = [D].[XsId] inner join [qp_sch_Banji] as [B] on [A].[BanJi] = [B].[Id] where 1=1";
            string str2 = "select count(A.id) from [stu_table_" + this.divsch.GetXueqi() + "_" + this.divsch.GetXueduan().Replace("上学期", "1").Replace("下学期", "2") + "] as [A] inner join [qp_sch_Xuesheng] as [C] on [A].[XsId] = [C].[Id] inner join [qp_sch_Stu_jtxx] as [D] on [A].[XsId] = [D].[XsId] inner join [qp_sch_Banji] as [B] on [A].[BanJi] = [B].[Id] where 1=1";
        ...
        protected void DelData_Click(object sender, EventArgs e)
        {
            ...
            string sql = "  Delete from qp_sch_Stu_jtxx  where ID in (" + str + ")";
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("删除家庭信息[" + this.CheckCbxNameDel() + "]", "家庭信息");
            this.DataBindToGridview(this.SortText.Value, this.CreateMidSql());
        }
        ...
        protected void UpdateData_Click(object sender, EventArgs e)
        {
            if (base.Request.QueryString["XsId"] != null)
            {
                base.Response.Redirect("Xueshengxinxi_jtxx_update.aspx?id=" + this.CheckCbxUpdate() + "&XsId=" + base.Request.QueryString["XsId"] + "&Xueduan=" + base.Request.QueryString["Xueduan"] + "&Xueqi=" + base.Request.QueryString["Xueqi"] + "");
            }
            else
            {
                base.Response.Redirect("Xueshengxinxi_jtxx_update.aspx?id=" + this.CheckCbxUpdate() + "&Banji=" + base.Request.QueryString["id"] + "&Xueduan=" + base.Request.QueryString["Xueduan"] + "&Xueqi=" + base.Request.QueryString["Xueqi"] + "");
            }
        }
    }
}
