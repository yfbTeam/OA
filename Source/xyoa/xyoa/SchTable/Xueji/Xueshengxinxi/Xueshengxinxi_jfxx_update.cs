namespace xyoa.SchTable.Xueji.Xueshengxinxi
{
    ...
    public class Xueshengxinxi_jfxx_update : Page
    {
        ...
        {
            string sql = string.Concat(new object[] { "Update qp_sch_Stu_jfxx     Set Xueqi='", this.Xueqi.SelectedValue, "',Xueduan='", this.Xueduan.SelectedValue, "',Shijian='", this.pulicss.GetFormatStr(this.Shijian.Text), "',Yuanyin='", this.pulicss.GetFormatStr(this.Yuanyin.Text), "' where   id='", int.Parse(base.Request.QueryString["id"]), "'" });
            this.List.ExeSql(sql);
            this.pulicss.InsertLog("修改家访信息[" + this.XsId.Text + "]", "家访信息");
            ...
            {
                base.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='Xueshengxinxi_jfxx.aspx?id=" + base.Request.QueryString["Banji"] + "&Xueduan=" + base.Request.QueryString["Xueduan"] + "&Xueqi=" + base.Request.QueryString["Xueqi"] + "'</script>");
            }
        }
