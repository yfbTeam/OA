namespace xyoa.SchTable.Chengji.Chaxun
{
    ...
    public class Chaxun_ListNj : Page
    {
        ...
        public void BindXq()
        {
            string sql = "select * from qp_sch_Nianji where  NianjiMc='" + this.pulicss.GetFormatStr(base.Request.QueryString["Id"]) + "' and Xueqi='" + this.Xueqi.SelectedValue + "'";
        ...
        public void DataBindToGridview()
        {
            ...
            this.LTongji.Text = string.Concat(new object[] { text, "<tr align=\"center\"> <td colspan=\"", num, "\"> <font style='font-weight: bold; color: red'>", this.Xueqi.SelectedValue, "", this.Xueduan.SelectedItem.Text, "&nbsp;&nbsp;年级:", base.Request.QueryString["Id"].ToString(), "&nbsp;&nbsp;考试类型:", this.Leixing.SelectedItem.Text, "</font></td> </tr>" });
            ...
            while (reader2.Read())
            {
                ...
                int num4 = 0;
                int num5 = 0;
                string str6 = "select sum(Chengji) as Mark,[A].[XsId] from qp_sch_Chengji as [A] inner join [stu_table_" + this.Xueqi.SelectedValue + "_" + this.Xueduan.SelectedValue.Replace("上学期", "1").Replace("下学期", "2") + "] as [C] on [A].[XsId] = [C].[XsId] inner join qp_sch_Banji as [D] on [D].[id] = [C].[BanJi]  where A.Kemu in (" + this.pulicss.GetChecked(this.Kemu) + ") and A.Leixing='" + this.Leixing.SelectedValue + "'    and A.Xueqi='" + this.Xueqi.SelectedValue + "'  and A.Xueduan='" + this.Xueduan.SelectedValue + "'  and D.Nianji='" + base.Request.QueryString["id"].ToString() + "'  group by [A].[XsId] order by Mark desc";
