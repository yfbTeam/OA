            ...
            while (list.Read())
            {
                ...
                while (reader2.Read())
                {
                    ...
                    if (((str4.IndexOf("," + this.Session["Username"] + ",") >= 0) || (str5.IndexOf("," + this.Session["Username"] + ",") >= 0)) || (str3 == ("" + this.Session["Username"] + "")))
                    {
                        ...
                        {
                            node2.Text = string.Concat(new object[] { "<a href='Chaxun_List.aspx?id=", num, "' target='nextrform'><font color=blue>", reader2["Mingcheng"].ToString(), "</font></a>" });
                        }
                        if (base.Request.QueryString["url"].ToString() == "2")
                        {
                            node2.Text = string.Concat(new object[] { "<a href='Chaxun_ListBj.aspx?id=", num, "' target='nextrform'><font color=blue>", reader2["Mingcheng"].ToString(), "</font></a>" });
                        }
                    }
                    else
                    {
