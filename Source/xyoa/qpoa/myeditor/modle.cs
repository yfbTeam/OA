        ...
        public void DataBindToGridview(string StrCheck)
        {
            ...
            if (StrCheck == "0")
            {
                ...
                while (list.Read())
                {
                    ...
                    this.ModelContent = string.Concat(new object[] { modelContent, "  <tr class=\"TableControl\">  <td align=\"middle\" class=\"menulines\" onclick=\"javascript:return click_model('CONTENT_", list["id"], "')\" style=\"cursor: hand\">", list["name"], "<input  type=\"hidden\" value=\"", str2, "\"  name=\"CONTENT_", list["id"], "\"/></td> </tr>" });
                    num++;
                    if (num == 1)
                    {
                        num = 0;
