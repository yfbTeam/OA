<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WorkFlowListSpYzY.aspx.cs" Inherits="xyoa.WorkFlow.WorkFlowListSpYzY" %>
<html>
...
<script>
    function chknull()
    {
			...
			{
			    if (!confirm("确定应用印章到表单中吗？"))
			   {
                return false;
               }
               else
               {
               //window.returnValue="<%=Request.QueryString["picname"] %>|" + document.getElementById('DropDownList1').value + "|使用人：<%=Session["realname"] %>，使用时间：<%=System.DateTime.Now.ToString() %>，使用步骤：<%=Request.QueryString["NodeName"] %>";
               }
			}
	}
	function _onchange()
...
<body class="newbody" onload="_onchange();">
