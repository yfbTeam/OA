...
function showContextMenu(event,type)
{
  ...
  case 1:
  	...
  	sMenu += getMenuRow("set_hong("+Process_ID+")", "可转换宏控件");
  	height += 20;
  	
  	sMenu += getMenuRow("set_dept("+Process_ID+")", "保密字段");
  	height += 20;

  	sMenu += getMenuRow("set_bitian("+Process_ID+")", "必填字段");
  	height += 20;
  	
  	sMenu += getMenuRow("set_condition("+Process_ID+")", "条件设置");
  	height += 20;

  	sMenu += getMenuRow("set_guolv("+Process_ID+")", "过滤设置");
  	height += 20;

  	sMenu += getMenuRow("set_yinzhang("+Process_ID+")", "必选印章");
  	height += 20;
...
var num = Math.random();
window.showModalDialog("WorkFlowName_show_add_node_savaps.aspx?str=" + str11 + "&tmp=" + num + "", "window", "dialogWidth:1px;DialogHeight=1px;status:no;scroll=yes;help:no");
...
{
//  loc_x=(screen.availWidth-780)/2;
//  loc_y=(screen.availHeight-700)/2;
//  //URL+="&GRAPH=1";
//  window.open(URL,"set_process","height=670,width=820,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,top="+loc_y+",left="+loc_x+",resizable=yes");
  
    var num = Math.random();
    window.showModalDialog("" + URL + "&tmp=" + num + "", "window", "dialogWidth:820px;DialogHeight=670px;status:no;scroll=yes;help:no");
    location.href = location.href;
}
...
function set_hong(Process_ID)
{
	LoadWindow("WorkFlowName_show_add_node_hkj.aspx?ID="+Process_ID);
}
...
{
	LoadWindow("WorkFlowName_show_add_node_bmzd.aspx?ID="+Process_ID);
}

function set_bitian(Process_ID)
{
	LoadWindow("WorkFlowName_show_add_node_btzd.aspx?ID="+Process_ID);
}

function set_guolv(Process_ID)
{
	LoadWindow("WorkFlowName_show_add_node_gl.aspx?ID="+Process_ID);
}

function set_yinzhang(Process_ID)
{
	LoadWindow("WorkFlowName_show_add_node_yz.aspx?ID="+Process_ID);
}
...
function Del_Process(Process_ID)
{
   ...
   {
     var num = Math.random();
    window.showModalDialog("WorkFlowName_show_add_node_del.aspx?id=" + Process_ID + "&tmp=" + num + "", "window", "dialogWidth:1px;DialogHeight=1px;status:no;scroll=yes;help:no");
    location.href = location.href;
    }
